...
ServerEvents.recipes(function(event) {
    ...
    console.log('[伪神之路·配方] 读取状态：' + enabled);

    if (!enabled) {
        console.log('[伪神之路·配方] 未开启，跳过所有配方');
        return;
    }

    console.log('[伪神之路·配方] 开始加载伪神机器配方...');
    ...
    voidAbsorbers.forEach(function(arr) {
        var tier = arr[0], plate = arr[1], hatch = arr[2];
    ...
    // 维度能量稳定枢纽 —— 终局发电机，梦魇合成
    gtr.nightmare_crafting('kubejs:dimensional_energy_stabilization_hub')
        .itemInputs(
            '128x gtladditions:heart_of_the_universe',
            '128x gtceu:annihilate_generator',
            '64x avaritia:singularity',
            '64x gtceu:dyson_sphere',
            '64x gtlcore:max_field_generator',
            '64x gtladditions:forge_of_the_antichrist',
            '32x gtceu:chaos_block',
            '32x kubejs:suprachronal_mainframe_complex',
            '16x gtlcore:max_electric_motor',
            '16x gtlcore:max_electric_pump',
            '16x gtlcore:max_conveyor_module',
            '16x gtlcore:max_robot_arm',
            '16x gtlcore:max_electric_piston',
            '16x gtlcore:max_sensor',
            '16x gtlcore:max_emitter'
        )
        .inputFluids(
            'gtceu:dimensionallytranscendentcrudecatalyst 5250000',
            'gtceu:orichalcum_plasma 1000000',
            'gtceu:naquadria 500000',
            'gtceu:lawrencium 250000',
            'gtceu:nobelium 250000'
        )
        .itemOutputs('gtlsupb:dimensional_energy_stabilization_hub')
        .duration(6000)
        .EUt(524288 * 5000);

    gtr.assembly_line('gtlsupb:abyssal_chemical_eroder')
    ...
    console.log('[伪神之路·配方] 所有伪神配方加载完毕');
});