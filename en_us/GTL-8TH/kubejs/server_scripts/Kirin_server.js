		...
		BlockEvents.broken(Ingredient.of(/gtceu:*_parallel_hatch/), (event) => {
			...
			if (currentParallel > maxAllowed) {
				...
				if (loadedAddons.thetornproductionline) {
					...
					let loreText = `"${compressionText} Actual number of parallelism: ${currentParallel} Place carefully Parallelism will be reset after mining/clicking"`
	...
	ServerEvents.recipes((event) => {
		...
		if (global.kirin.enableSafeFixes) {
			...
			// 无限编程电路元件包
			{
				...
				let nbtString = `{RepairCost:0,amts:[L;${amtsArray}],display:{Name:'{"text":"§rInfinitely Programmable Circuit Component Package"}'},ic:33L,internalCurrentPower:20000.0d,keys:[${keysString}]}`
		...
		if (global.kirin.enableGameBreakingRecipes) {
			// 锻铁锭直接烧成钢锭
			event.smelting('gtceu:steel_ingot', 'gtceu:wrought_iron_ingot', 0, 1)
			// 黑中子素锭洗白成白中子素锭
