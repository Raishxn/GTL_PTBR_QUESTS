(function() {
    ...
    let config;
    try { config = JsonIO.read('kubejs/config/[config]Special for production line twisters.json'); if (typeof config !== 'object') throw new Error(); }
    ...
    log('[Special Offer for Line Twisters] ⚡ Special recipe script is loading...');
    log(`[产线扭曲者特供] 全局启用: ${TwistedLine.enableBlastScript} | 时间加速: ${TwistedLine.enableTimeAcceleration}`);

    const SPEED_TIER = [{name:"Star Core Beginner",range:3,tip:"⚡ Lingering light, primary space-time resonance"},{name:"Nebula Intermediate",range:6,tip:"⚡ Nebula flows, intermediate dimensions fold"},{name:"Galaxy Advanced",range:9,tip:"⚡ Galaxy surges, high-level time and space jumps"},{name:"Star Dome Ultimate",range:12,tip:"⚡ The starry sky explodes and the ultimate time is annihilated"}];
    let OFFSETS = [];
 ...
 const isGTLAdditions31 = (function() {
    ...
    if (!TwistedLine.enableBlastScript) {
        ...
        if (TwistedLine.enableLogReport) console.log('[Special for Production Line Twisters] Recipe modification skipped due to the global switch being turned off.');
        return;
    }
    ...
    (function() {
        ...
        log('[Special for those with production line distortion] ④ The mold/field shape has been replaced with a programming circuit, affecting approx.'+moldImpact+'recipes');
    ...
    (function() {
        ...
        log('[Specially available to those with production line distortion] ⑤ The lens has been replaced with a programming circuit (decreasingly starting from 32), affecting approximately' + lensImpact + 'recipes');
...
if (TwistedLine.enableFluidSolidifierAll) {
    ...
    (function() {
        const recipes = [
            { id: "hongmengitems", inputFluids: ["gtceu:cosmic_element 2147483647"], inputs: ["16384x kubejs:leptonic_charge","128x gtladditions:arcanic_astrograph","16384x gtladditions:astral_array"], cellNBT: buildCellNBTForItems('Hongmeng+ component package', HONGMENG_ITEMS), circuit: 32, euTier: "MAX" },
            { id: "plasmafluids", inputFluids: ["gtceu:cosmic_element 2147483647"], inputs: ["16384x kubejs:quantum_chromodynamic_charge","128x gtladditions:fuxi_bagua_heaven_forging_furnace","16384x gtladditions:astral_array"], cellNBT: buildCellNBTForFluids('Plasma component package', PLASMA_FLUIDS), circuit: 32, euTier: "MAX" },
            { id: "componentitems", inputFluids: ["gtceu:cosmic_element 2147483647"], inputs: ["2048x gtlcore:component_assembly_line_casing_max","1024x gtceu:component_assembly_line","16384x gtladditions:astral_array","16384x gtlcore:max_electric_motor","16384x gtlcore:max_electric_pump","16384x gtlcore:max_conveyor_module","16384x gtlcore:max_robot_arm","16384x gtlcore:max_electric_piston","16384x gtlcore:max_emitter","16384x gtlcore:max_sensor","16384x gtlcore:max_field_generator"], cellNBT: buildCellNBTForItems('component package', COMPONENT_ITEMS), circuit: 32, euTier: "MAX" }
        ];
        ...
        if (nanoswarmItems.length > 0) {
            let nanoswarmCellNBT = buildCellNBTForItems('Nano Swarm Component Package', nanoswarmItems);
        ...
        if (TwistedLine.enableExclusiveContent) {
            ...
            if (crystalGems.length > 0) {
                let crystalCellNBT = buildCellNBTForItems('Crystal component package', crystalGems);
                event.shapeless(
    ...
    (function() {
        ...
        const nbtForge = `{BlockEntityTag:{runningSecs:144000L},display:{Name:'{"text":"The Forge of the Final False God","color":"#FF4500","bold":true,"italic":true}',Lore:['"§6§lFalse God\'s Forging·Ultimate Form"','"§e§nAfter the endless twists and turns of the quantum fluctuation matrix, the endless power has created the foundation of the false god"','"§7§mAll things in the EU that swallowed 9.2E in one second are coming to an end"','"§5§ocarries all things and breaks through the shackles of the world"','"§c§kBurn everything in the world..."']}}`;
        const recipes = [
    ...
    ServerEvents.loaded(event => { let stats=TwistedLine.blast_stats; if(!stats) return; event.server.getPlayers().forEach(p=>{ if(!stats.enabled) p.tell(Text.gold("§l[Special for Line Twister] ⚡ Special recipe script is disabled")); else p.tell(Text.gold("§l[Special for Line Twisters] ⚡ The special recipe script has taken effect!\n → Loading time:"+stats.loadTimeMs+"ms\n → New recipe:"+stats.addedRecipes)); }); });
    ...
    if (!stats || !stats.enabled) {
        p.tell("§l⚠ No special recipe modifications applied");
    } else {
        p.tell("§l⚡ Special recipe modification status\n┌───────────────────────────────────────\n│▪ Loading time: §a" + stats.loadTimeMs + "ms §7| New: §a" + stats.addedRecipes + "│▪ Global: §aenabled\n├───────────────────────────────────────");
        p.tell("└─────────────────────────────────────────");
        p.tell("§e⚡ Enter §6/assembly_line_distorter §eto view all switch status and switching commands (OP permission required)");
    }
    if (TwistedLine.enableBlastScript && TwistedLine.enableTimeAcceleration) {
        ...
        p.tell("§6\"Time and Space∞ Overclocking Technology\"" + (d.getBoolean("ttw_enabled") ? "§ais activated∞" : "§cis not activated∞") + "| Gear:" + cur.name + " §7(" + cur.range + "grid)\n §eOperation: K key switch | Shift+K shift");
    } else if (!TwistedLine.enableBlastScript) {
        p.tell(Text.red("The time acceleration feature has been disabled by the master switch"));
    } else if (!TwistedLine.enableTimeAcceleration) {
        p.tell(Text.red("Time acceleration has been disabled"));
    }
    p.tell("§d=====================================");
});
    ...
    NetworkEvents.dataReceived('ttw_toggle_pressed', e=>{ if(!TwistedLine.enableBlastScript||!TwistedLine.enableTimeAcceleration) return; let p=e.player; if(!p) return; let d=p.persistentData; let cur=d.getBoolean('ttw_enabled'); d.putBoolean('ttw_enabled',!cur); p.setStatusMessage(!cur?'§6『Space-Time∞Overclocking Technology』 §a⚡ The core engine ignited successfully∞ and the space-time flow rate is full!':'§6"Space-time∞ Overclocking Technology" §c⚡ The core engine shuts down in an emergency and space-time returns to normal flow rate!'); p.playSound(!cur?'minecraft:item.nether_star.use':'minecraft:item.nether_star.break'); });
    ...
    BlockEvents.rightClicked('expatternprovider:fishbig', e => {
        ...
        e.player.tell('§5You have obtained §dBox of Creation §5This is the gift of the end');
        e.cancel();
    });
})();
...
(function() {
    var SWITCHES = [
        { key: "enableBlastScript", name: "global master switch", cmd: "overall situation", desc: "Global master switch, false will skip all recipe modifications" },
        { key: "enableSimpleIngotProcessing", name: "Easy tablet handling", cmd: "Simple ingot", desc: "Simple ingot processing section" },
        { key: "enableSimpleLineProcessing", name: "Simple production line processing", cmd: "Simple production line", desc: "Production line simple processing section" },
        { key: "enableFluidSolidifierAll", name: "fluid solidifier", cmd: "Curing device", desc: "Fluid solidifier section" },
        { key: "enableExtractorOverride", name: "Extraction Machine", cmd: "Extraction Machine", desc: "Extraction machine section" },
        { key: "enableMoldAndSpecialRecipes", name: "Mold replacement circuit", cmd: "Mold", desc: "Molds replaced with programmed circuit blocks" },
        { key: "enableLensReplacement", name: "Lens replacement circuit", cmd: "lente", desc: "Lens replaced with programming circuit block" },
        { key: "enableTimeAcceleration", name: "hyperspace acceleration", cmd: "time and space", desc: "Hyperspace Acceleration Section" },
        { key: "enableFusionRecipes", name: "nuclear fusion", cmd: "fusion", desc: "Nuclear fusion reactor sector" },
        { key: "enableParticleColliderRecipes", name: "particle collision", cmd: "Collision", desc: "super particle collision plate" },
        { key: "enableGeneratorRecipes", name: "Explosion to generate electricity", cmd: "generate electricity", desc: "Explosive power generation sector" },
        { key: "enableCircuitPackRecipes", name: "circuit packaging", cmd: "circuit package", desc: "Workbench circuit packaging section" },
        { key: "enableFurnaceBlastAcceleration", name: "furnace acceleration", cmd: "furnace acceleration", desc: "Furnace and blast furnace acceleration tiles" },
        { key: "enableComponentPackRecipes", name: "component package", cmd: "component package", desc: "Component package section" },
        { key: "enableInfinityCellRecipes", name: "infinite components", cmd: "infinite components", desc: "Unlimited components section" },
        { key: "enableGTLAdditionsUpgrade", name: "GTLAdditionsUpgrade", cmd: "gtladd", desc: "GTLAdditions upgrade section" },
        { key: "enableDistortRecipes", name: "neutron activation", cmd: "neutron", desc: "Neutron Activation Section" },
        { key: "enableStellarForgeRecipes", name: "remove jar", cmd: "jar", desc: "Remove the jar tile" },
        { key: "enableContentOptimizationRecipes", name: "Content optimization", cmd: "optimization", desc: "Content optimization section" },
        { key: "enableFlight", name: "flight", cmd: "flight", desc: "Flight section" },
        { key: "enableInvulnerable", name: "Invincible", cmd: "Invincible", desc: "Invincible section" },
        { key: "enableExclusiveContent", name: "exclusive content", cmd: "Exclusive", desc: "Exclusive content section for production line twisters" },
        { key: "enableAlloyBlastSmelterRecipes", name: "Alloy Smelting Furnace", cmd: "alloy furnace", desc: "Alloy smelting furnace blasting section" },
        { key: "enableSimpleAggregationRecipes", name: "Simple polymerization device", cmd: "polymerization", desc: "Simple aggregation device section" },
        { key: "enablePlasmaCondenserRecipes", name: "Simple plasma condensation", cmd: "plasma", desc: "Simple plasma condensation plate" },
        { key: "enableGTCEUElementCompletion", name: "GTCEu element completion", cmd: "element completion", desc: "GTCEu element completion switch" },
        { key: "enableLogReport", name: "log report", cmd: "log", desc: "Control all section statistics and logs uniformly with this switch." },
        { key: "enableGtDistortedAePack", name: "GT Twisted AE Component Pack", cmd: "gt twisted ae", desc: "GT Twisted AE Component Pack Module Switch" },
        { key: "enableGTLsupbRecipes", name: "GTLsupb synthesis", cmd: "gtlsupb", desc: "GTLsupb machine synthesis recipe module switch" }
    ];

    function readConfig() {
        try {
            var config = JsonIO.read('kubejs/config/[config]Special for production line twisters.json');
    ...
    function writeConfig(config) {
        JsonIO.write('kubejs/config/[config]Special for production line twisters.json', config);
    }
    ...
    ServerEvents.commandRegistry(function(event) {
        ...
        event.register(Commands.literal("assembly_line_distorter").then(Commands.literal("help").executes(function(ctx) {
            ...
            if (!player) return 1;
            player.tell("§l=== Production Line Twister Command List ===");
            player.tell("§e/assembly_line_distorter §7- Displays all switch states");
            player.tell("§e/assembly_line_distorter help §7- Show this help");
            for (var i = 0; i < SWITCHES.length; i++) {
                var sw = SWITCHES[i];
                player.tell("§e/assembly_line_distorter_" + sw.cmd + "§7- Switch" + sw.name + "：" + sw.desc);
            }
            return 1;
        })));
        ...
        event.register(Commands.literal("assembly_line_distorter").executes(function(ctx) {
            ...
            if (!player || !player.isOp()) {
                if (player) player.tell("§cYou do not have permission to use this command!");
                return 1;
            }
            var config = readConfig();
            player.tell("§l=== Production line twister module status ===");
            var hasAny = false;
            for (var i = 0; i < SWITCHES.length; i++) {
                var sw = SWITCHES[i];
                ...
                hasAny = true;
                var color = val ? "§a" : "§c";
                var status = val ? "✔ Turn on" : "✘ Close";
                var desc = config["_comment_" + sw.key] || sw.desc;
                player.tell(color + sw.key + " : " + status + " §7- " + desc);
            }
            if (!hasAny) player.tell("§7(no configuration items)");
            player.tell("§7configuration file: kubejs/config/[config]Special for production line twisters.json");
            player.tell("§eType /assembly_line_distorter help to view all available commands");
            return 1;
        }));
        ...
        for (var i = 0; i < SWITCHES.length; i++) {
            ...
            (function(sw) {
                event.register(Commands.literal(cmdName).executes(function(ctx) {
                    ...
                    player.tell("§aswitched" + sw.name + " : " + (newVal ? "turn on" : "closure") + " §7- " + sw.desc);
                    return 1;
                }));
            })(sw);
        }
