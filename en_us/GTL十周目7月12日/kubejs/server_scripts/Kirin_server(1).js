...
;(function() {
	...
	if(!KIRIN_CONFIG){
		console.error("Kirin's personal product - the server has not detected the configuration file, please check the installation!")
		return
	}
	//其它私货是否加载
	const loadedAddons = {
	...
	console.log('Private goods loading status:')
	...
	if (loadedAddons.gtladd3) {
		console.warn('GtlAdditions3.0+ detected, this private product has not been tested for compatibility, continuing to load may cause problems!')
	}
	//所有流体
	...
	if (global.kirin.enableSafeFixes) {
		...
		BlockEvents.rightClicked(Ingredient.of(/gtceu:*_parallel_hatch/), (event) => {
			...
			if (currentParallel > maxAllowed) {
				player.tell(Text.aqua('You are trying to open a parallel control bin with a parallel number that exceeds the maximum number of parallel controls for this level of parallel control bin. Are you sure you want to do this?'))
				player.tell(Text.red('This will cause the parallel number of the parallel control bin to be reset to the maximum parallel number of the parallel control bin at that level!'))
				player.tell(Text.darkGray('Hold the stick in your main hand to ignore this warning and open the parallel control chamber.'))
				event.cancel()
			}
		})
		//大并行控制仓原样掉落
			...
			if (currentParallel > maxAllowed) {
				...
				if (loadedAddons.thetornproductionline) {
					...
					if (currentParallel === maxAllowed * 4) {
						compressionText = 'One layer of compression'
					} else if (currentParallel === maxAllowed * 16) {
						compressionText = 'Double compression'
					} else if (currentParallel === maxAllowed * 64) {
						compressionText = 'triple compression'
					}
					let loreText = `"$ {compressionText} Actual number of parallels: $ {currentParallel} Place with caution Parallelism will be reset after mining/clicking"`
	...
	ServerEvents.recipes((event) => {
		...
		if (global.kirin.enableSafeFixes) {
			...
			// 无限编程电路元件包
			{
				...
				let nbtString = `{RepairCost:0,amts:[L;${amtsArray}],display:{Name:'{"text":"§rinfinite programming circuit component package"}'},ic:33L,internalCurrentPower:20000.0d,keys:[${keysString}]}`
		...
		if (global.kirin.enableSignificantBalanceChanges) {
			...
			// 虚空流体钻机加强
			{
				...
				drillerList.forEach((element) => {
					gtr.void_fluid_drilling_rig(`kirin:${element[0].split(':')[1]}_overworld_void_fluid_drilling`)
						...
						.duration(100)
					gtr.void_fluid_drilling_rig(`kirin:${element[0].split(':')[1]}_nether_void_fluid_drilling`)
						.notConsumable(element[0])
						.notConsumable('kubejs:nether_data')
						.outputFluids(netherDataFluidList.map((fluid) => (fluid = `${fluid[0]} ${fluid[1] * element[1]}`)))
						.EUt(GTValues.VA[element[2]])
						.duration(100)
					gtr.void_fluid_drilling_rig(`kirin:${element[0].split(':')[1]}_end_void_fluid_drilling`)
						.notConsumable(element[0])
