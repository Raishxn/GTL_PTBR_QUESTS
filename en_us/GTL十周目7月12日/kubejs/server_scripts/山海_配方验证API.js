...
var LOG_PREFIX = '§d[Recipe Verification API] §r';
...
function getGtrObject() {
    ...
    for (var i = 0; i < possibleGtrSources.length; i++) {
        if (possibleGtrSources[i]) {
            debug('from source' + i + 'Get the gtr object');
            return possibleGtrSources[i];
        }
    }
    
    warn('Unable to obtain gtr object, some verification functions may be limited');
    return null;
}
...
function getAllRecipeArrays() {
    ...
    // （以'recipe'、'recipes'、'formula'等关键词结尾的变量）
    var recipeKeywords = ['recipe', 'recipes', 'formula', 'ass', 'universal', 'sup'];
