...
ServerEvents.recipes(function(e) {
    ...
    // 日志函数（本地定义，避免跨文件依赖）
    var LOG_PREFIX = '§b[Shanhai formula library] §r';
        ...
        try {
            ...
            if (typeof arg1 === 'string' && typeof arg2 === 'string' && typeof arg3 === 'function') {
                ...
                if (!engineIsRecipeEnabled(arg2, recipeObj.defaultEnabled, recipeLoadConfig)) {
                    DShanhaiRecipeEngine.recordSkippedRecipe(arg1, arg2, 'Recipe loading disabled, skipped');
                    return true;
                }
                try {
                    arg3(recipeObj);
                    DShanhaiRecipeEngine.recordRecipe(arg1, true, arg2, 'callback registration completed');
                    return true;
                } catch (callbackErr) {
            ...
            console.error('[safeAddRecipe] Unsupported call parameter');
            return false;
        } catch (err) {
            var recipeId = (typeof arg2 === 'string') ? arg2 : (arg1 && arg1.id ? arg1.id : 'unknown');
            console.error('[safeAddRecipe compat] ' + recipeId + 'Registration failed:' + err);
            return false;
        }
    };
    ...
    var debug = (typeof debug !== 'undefined') ? debug : function(m) { console.log('[Recipe Library]' + m); };
    var broadcastRecipeError = (typeof broadcastRecipeError !== 'undefined') ? broadcastRecipeError : function(type, id, msg) { console.error('[recipe error]' + type + ': ' + id + ' - ' + msg); };
...
// 配方验证函数
    ...
    assrecipes.forEach(function(recipe) {
        ...
        if (!validation.valid) {
            console.error('❌ Recipe verification failed:' + recipe.id + ' (' + recipe.type + ') - ' + validation.error);
...
//宏产世界碎片
    ...
    try {
        ...
        if (outputStacks.length === 0) {
            console.log('No valid ore product output, skip star_core_stripper_ores');
        } else {
            safeAddRecipe('star_core_stripper', 'dishanhai:star_core_stripper_ores', function() {
                gtr.star_core_stripper('dishanhai:star_core_stripper_ores')
                    ...
                    .EUt(max)
                    .duration(200);
                console.log('output' + outputStacks.length + 'Ore product (impurity prefix excluded)');
            },{defaultEnabled:false});
        }
    } catch(err) {
        console.log(err);
...
// 以下配方需要 gt_shanhai 模组注册的配方类型
    ...
    const assemblerRecipes = [
        { id: 'heidonqidian', type:'qft', name: 'Black hole singularity assembly',circuit:1,notConsumable:'16x gtladditions:forge_of_the_antichrist', itemInputs: ['kubejs:naquadria_charge','64x kubejs:time_dilation_containment_unit','64x kubejs:charged_triplet_neutronium_sphere'], itemOutputs: ['1920x kubejs:contained_reissner_nordstrom_singularity'], EUt:uxv, duration: 20 },
        { id: 'qiyiqid', type:'qft', name: 'Strange matter assembly', circuit:2,notConsumable:'16x gtladditions:forge_of_the_antichrist',itemInputs: ['15x gtceu:degenerate_rhenium_dust','kubejs:leptonic_charge','kubejs:contained_high_density_protonic_matter'], itemOutputs: ['30x kubejs:contained_exotic_matter'], EUt: UXV, duration: 20 },
        { id: 'gaomiduqidian', type:'qft', name: 'High density material assembly',circuit:3,notConsumable:'16x gtladditions:forge_of_the_antichrist', itemInputs: ['kubejs:leptonic_charge','kubejs:time_dilation_containment_unit','kubejs:charged_triplet_neutronium_sphere'], itemOutputs: ['30x kubejs:contained_high_density_protonic_matter'], EUt: UXV, duration: 20 },
        { id: 'niumanheidonqidian', type:'qft', name: 'Kerr Newman Singularity Assembly',circuit:4,notConsumable:'16x gtladditions:forge_of_the_antichrist' ,itemInputs: ['kubejs:time_dilation_containment_unit','64x kubejs:charged_triplet_neutronium_sphere'], inputFluids: ['gtceu:uu_matter 10000'], itemOutputs: ['30x kubejs:contained_kerr_newmann_singularity'], EUt: UXV, duration: 20 }
...
// ========== 分子解构配方组 ==========
    ...
    const molecularRecipes = [
        { id: 'ytyh', type: 'molecular_deconstruction', name: 'Time and space powder deconstruction', itemInputs: ['gtceu:spacetime_dust'], outputFluids: ['gtceu:spacetime 144'], EUt: opv, duration: 20 },
        { id: 'ciyues', type: 'molecular_deconstruction', name: 'Stellar magnetic fluid deconstruction', itemInputs: ['gtceu:magnetohydrodynamicallyconstrainedstarmatter_block'], outputFluids: ['gtceu:magnetohydrodynamicallyconstrainedstarmatter 1296'], EUt:max, duration: 20 },
        { id: 'ytciwuz', type: 'molecular_deconstruction', name: 'Deconstruction of magnetic matter', itemInputs: ['gtceu:magmatter_dust'], outputFluids: ['gtceu:magmatter 144'], EUt: 2147483647, duration: 20 },
        {id:'magnetohydrodynamicallyconstrainedstarmatter_dust', type: 'molecular_deconstruction', name:'Deconstruction of stellar matter constrained by magnetic fluid',itemInputs:['gtceu:magnetohydrodynamicallyconstrainedstarmatter_dust'],outputFluids:['gtceu:magnetohydrodynamicallyconstrainedstarmatter 144'],EUt:max, duration: 20}
    ];
    ...
    const compressorRecipes = [
        { id: 'tiny_magmatter_dust', name: 'Magnetic material powder compression 1', itemInputs: ['9x gtceu:tiny_magmatter_dust'], itemOutputs: ['gtceu:small_magmatter_dust'], EUt: 2147483, duration: 20 },
        { id: 'magmatter_dust', name: 'Magnetic material powder compression 2', itemInputs: ['9x gtceu:small_magmatter_dust'], itemOutputs: ['gtceu:magmatter_dust'], EUt: 21474836, duration: 20 }
    ];
    ...
    if(Platform.isLoaded('appflux')){
        ...
        debug('The appflux module is not loaded and related recipes are skipped.');
    }
...
//==============  通用配方 =================(wx,wuxian,♾️,wx)
    var dishanhai_timer = DShanhaiRecipeEngine.startTimer('Shan Hai’s ♾️Item Recipes');
...
// 创造模块 - 修复版
    ...
    var timercre = DShanhaiRecipeEngine.startTimer('Founding Reality Modification Module')
...
// 混沌炼金
    ...
    var timer_electrolyzer = DShanhaiRecipeEngine.startTimer('Electrolyzer')
...
// ========== ae2_overclocked 模组配方 ==========
    ...
    // ========== 输出物品盘配方 ==========
    info('📀 Start generating super AE package recipe...');
    ...
    try {
        ...
        info('✅ Super AE package recipe has been generated id= dishanhai:super_ae_pack NBT_len=' + (superAENBT ? superAENBT.length : 'null'));
    } catch(err) {
        error('❌ Super AE package recipe generation failed:' + err.message);
    }
    ...
    try {
        info('🎨 Start generating unlimited dye component pack pro recipes...');

        var dyeItemsList = Ingredient.of('#forge:dyes').getItemIds();
        if (!dyeItemsList || dyeItemsList.length === 0) {
            throw new Error('Dye item not found, tag # forge:dyes may be empty');
        }
        var dyeSeen = {};
        var dyeUniqueList = [];
        ...
        info('🎨 Retrieved from the tag # forge:dyes' + dyeItemsList.length + 'dye items');
        gtr.assembler(dyeRecipeId)
            .circuit(1)
            .itemInputs('minecraft:dandelion')
            .itemOutputs(DShanhaiSDA.create('unlimited dye component pack pro').infinityOutputs(dyeItemsList).lore([
                '§7Unlimited component pack containing all dye items',
                '§7Dye type: §e' + dyeItemsList.length + '§7species',
                '§7Each dye is stored in an unlimited element package',
                '§8Shanhai private goods v2.2'
            ]).build())
            .duration(200)
            .EUt(LV);

        try { DShanhaiRecipeEngine.recordRecipe(dyeRecipeType, true, dyeRecipeId, ''); } catch(ignored) {}
        info('✅ Infinite dye component pack pro recipe has been generated');
    } catch(err) {
        error('❌ Unlimited dye component package pro recipe generation failed:' + err.message);
        try { DShanhaiRecipeEngine.recordRecipe(dyeRecipeType, false, dyeRecipeId, err.message); } catch(ignored) {}
    }

    // ========== 天基大礼包配方 ==========
    info('🔧 Start generating space-based gift package recipes...');
    ...
    try {
        ...
        info('✅ Tianji gift package recipe has been generated');
    } catch(err) {
        error('❌ Tianji gift package recipe generation failed:' + err.message);
    }

    // ========== 猪咪大礼包配方 ==========
    info('🔧 Start generating piggy gift packages...');
    ...
    try {
        ...
        gtr.assembler(templaterecipeId)
            ...
            .duration(200)
            .EUt(LV);
        info('✅ Piggy gift pack recipe has been generated:' + templaterecipeId);
    } catch(err) {
        error('❌ Failed to generate piggy gift package recipe:' + err.message);
    }

    // ========== 奇点数据中枢建材存储阵列配方 ==========
    info('🔧 Singularity data center building materials storage array...');
    try {
    ...
    try {
        ...
        info('🎨Get it' + dyeItemsList.length + 'kind of items');
        gtr.assembler('dishanhai:super_disk_array_naquadah_pack')
            .circuit(1)
            .itemInputs("dishanhai:dishanhai")
            .itemOutputs(DShanhaiSDA.create('Integrated circuit DASDA')
                .infinityOutputs(dyeItemsList)
                .lore([
                    "Internal construction of integrated circuit related items/fluids",
                    '§7Automatically inline unlimited storage elements',
                    '§7Number of entries: §e' + dyeItemsList.length,
                    '{ultimate} The God of Mountains and Seas Private Goods v2.7.3{/}'
                ])
                .build())
            .duration(200)
            .EUt(LV);
        ...
        error('❌ Unlimited component package recipe generation failed:' + err.message);
    ...
    try {
        info('🎨 Start generating unlimited gtceu fluid component package recipes...');
        ...
        if (!gtceuFluidList || gtceuFluidList.length === 0) {
            throw new Error('No fluids found for gtceu module');
        }
        info('🎨 found' + gtceuFluidList.length + 'species gtceu fluid');
        ...
        gtr.assembler(gtceuFluidRecipeId)
            .circuit(1)
            .itemInputs('minecraft:dandelion')
            .itemOutputs(DShanhaiSDA.create('super disk array')
                .infinityOutputs(gtceuFluidList)
                .lore([
                    '§7Unlimited component package containing all gtceu fluids',
                    '§7Fluid type: §e' + gtceuFluidList.length + '§7species',
                    '§7Each fluid is stored in unlimited element packages',
                    '§8Shanhai private goods v2.7'
                ])
                .build())
            .duration(200)
            .EUt(LV);
        ...
        info('✅ Unlimited gtceu fluid component pack recipe has been generated');
    } catch(err) {
        error('❌ Unlimited gtceu fluid component package recipe generation failed:' + err.message);
    ...
    */
    ...
    try {
        info('🎨 Start generating unlimited plasma fluid component package recipes...');
        ...
        if (!plasmaFluidList || plasmaFluidList.length === 0) {
            throw new Error('plasma fluid not found');
        }
        info('🎨 found' + plasmaFluidList.length + 'plasma fluid');
        ...
        gtr.assembler(plasmaFluidRecipeId)
            .circuit(1)
            .itemInputs('minecraft:dandelion')
            .itemOutputs(DShanhaiSDA.create('Unlimited oil component package')
                .infinityOutputs(plasmaFluidList)
                .lore([
                    '§7Unlimited component pack containing all oils',
                    '§7Item type: §e' + plasmaFluidList.length + '§7species',
                    '§7Each item is stored in an infinite component pack',
                    '§8Shanhai private goods v2.7'
                ])
                .build())
            .duration(200)
        ...
        info('✅ Unlimited plasma fluid component package recipe has been generated');
    } catch(err) {
        error('❌ Unlimited plasma fluid component package recipe generation failed:' + err.message);
    ...
    */
    ...
    info('🔬 Start registering for photon matrix etching recipe...');
    ...
    info('🔬 Start registering for Dimension Focus Laser Etching Formula...');
    ...
    info('⭐ Start registering Star Flame Jump Plasma Recipe...');
