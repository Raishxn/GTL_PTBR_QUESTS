...
(function() {
    ...
    // 环境检测
    console.log('§6[Mountain and Sea Protection Layer] §aEnvironmental Test Passed §r');
    ...
    function sealAPI(apiObj, apiName) {
        ...
        try {
            ...
            console.log('§6[Mountain and Sea Protective Layer] §eWarning:' + apiName + 'Use backup protection scheme §r');
        }
    ...
    console.log('§6[Mountain and Sea Protection Layer] §aBasic protection layer has been loaded §r');
})();
// ==================== 基础保护层结束 ====================

var Version = '2.7.4fix (log system version 2.7.3)'//主版本与日志系统版本
...
// ---------------- 日志模块 ----------------
var LOG_PREFIX = '§b[Shanhai private goods] §r';
...
Timer.prototype.end=function(){
    ...
    info('⏱️ ' + this.name + 'time consuming:' + ms + 'ms');
    return ms;
};
    ...
    return function protectedFunction() {
        try {
            ...
            if (callCount > options.maxCallPerSecond) {
                error(`API调用频率过高: ${apiFunction.name || 'anonymous function'}，当前 ${callCount}/秒，限制 ${options.maxCallPerSecond}/秒`);
                throw new Error('API call frequency is too high, please try again later');
            }
            ...
            if (paramValidators) {
                for (var i = 0; i < paramValidators.length; i++) {
                    ...
                    if (validator) {
                        args[i] = validator(args[i], 'parameter' + (i + 1));
                    }
                }
            }
            ...
            if (options.requireOp && typeof Server !== 'undefined') {
                ...
                if (!hasOp) {
                    throw new Error('This API requires OP permission to access');
                }
            }
            ...
            if (options.logPerformance) {
                ...
                debug(`API ${apiFunction.name || 'anonymous function'} 执行时间: ${endTime - startTime}ms`);
            }
            ...
            // 错误处理
            error(`API调用失败: ${apiFunction.name || 'anonymous function'} - ${err.message}`);
            
            // 如果是验证错误或权限错误，直接抛出
            if (err.message.includes('parameter') || err.message.includes('Permissions') || err.message.includes('frequency')) {
                throw err;
            }
            
            // 其他错误返回安全值
            if (options.defaultValue !== undefined) {
                warn(`API ${apiFunction.name || 'anonymous function'} 出错，返回默认值: ${options.defaultValue}`);
...
function initializeProtection() {
    info('Initialize API protection system...');
    ...
    info('API protection system initialization completed');
}
...
function getFixedColorText(text, colorCode) {
    ...
    if (typeof text !== 'string') {
        console.error('[Shanhai private goods] getFixedColorText: The text must be a string, use the default text');
        text = 'Invalid text';
    }
...
function getAlternatingColorText(text, color1, color2) {
    // 防御性编程：确保输入有效
    if (typeof text !== 'string') {
        console.error('[Shanhai private goods] getAlternatingColorText: The text must be a string, use the default text');
        text = 'Invalid text';
    }
...
var newShanhaiAPI = {
        ...
        [
            ...
            function(p) { 
                if (p !== undefined && typeof p !== 'string') {
                    throw new Error('Parameter msg must be string or undefined, actual type:' + typeof p);
                }
                return p;
            }
        ],
    ...
    infinityCell: function(cellString, type) {
        ...
        if (!cellString || typeof cellString !== 'string') {
            throw new Error('cellString parameter must be a string');
        }
        
        // 检查是否为无限单元格格式
        if (!cellString.includes('@')) {
            throw new Error('Infinite cell formats must contain the @ symbol, such as " expatternprovider:infinity_cell @ gtceu:hydrogen "');
        }
        
        // 解析物品字符串
        var parsed = parseItemStringCellAPI(cellString);
        if (!parsed) {
            throw new Error('Unable to parse infinite cell format:' + cellString);
        }
        
        // 验证是否为无限单元格
        if (!parsed.id.includes('infinity_cell')) {
            warn('[shanhaiAPI.infinityCell] Warning: The item ID does not contain "infinity_cell", but the format contains the @ symbol:' + cellString);
        }
...
if (global.shanhaiAPI && typeof global.shanhaiAPI === 'object') {
    ...
    info('The existing Shanhai API has been merged: merge' + mergedCount + 'attributes, override' + overriddenCount + 'attributes');
} else {
    info('Initialize the new Shanhai API');
}
...
global.shanhaiRecipeAPI = {
     ...
     * @returns {string} 彩色文本
     * @example
     * let rainbow = global.shanhaiRecipeAPI.getRandomRainbowText("Mountain and sea private goods");
     ...
     * @returns {string} 彩色文本
     * @example
     * let staticRandom = global.shanhaiRecipeAPI.getStaticRandomText("Mountain and sea private goods", "myseed");
     ...
     * @returns {string} 彩色文本
     * @example
     * let sessionRandom = global.shanhaiRecipeAPI.getSessionRandomSingleColorText("Mountain and sea private goods");
     ...
     * @returns {string} 彩色文本
     * @example
     * let fixed = global.shanhaiRecipeAPI.getFixedColorText("Mountain and sea private goods", "§c");
     ...
     * @returns {string} 彩色文本
     * @example
     * let alternating = global.shanhaiRecipeAPI.getAlternatingColorText("Mountain and sea private goods", "§c", "§9");
    ...
    getSummary: function() {
        ...
        var summary = "Shanhai Private Product Formula Statistics";
        summary += "total:" + stats.total + "recipes";
        summary += "√Success:" + stats.success + "indivual";
        summary += "×Failure:" + stats.failed + "indivual";
        
        if (stats.errors.length > 0) {
            summary += "Warning: Recipe library error feedback contact qq: 1982932217";//自行替换
            summary += "The current version of God’s private goods:" + Version + "\n";
            summary += "X failure example:";
            ...
            if (stats.errors.length > 5) {
                summary += "..besides" + (stats.errors.length - 5) + "errors";
            }
            
            summary += "Some recipes failed to load, please notify the server administrator to check the logs";
        }
    ...
    reset: function() {
        ...
        typeFailed = 0;
        info('Recipe statistics have been reset');
    },
     ...
     *     console.log('Shanhai Private Goods has been loaded');
     * }
     */
    isLoaded: function() {
    ...
    findRecipeById: function(id) {
        ...
        console.log('§e[shanhairecipeAPI] shanhaiRecipeControlAPI.findRecipeById is not available');
        return null;
    },
    ...
    getRecipeDetails: function(recipeOrId) {
        if (typeof recipeOrId === 'string') {
            ...
            if (!result) return 'No recipe information';
     ...
     * console.log('Set enabled status results:', success);
     */
     ...
     * console.log('Set default value results:', success);
     */
    setRecipeDefault: function(recipeId, defaultValue) {
        console.log('§e[Shanhai Recipe API] setRecipeDefault function has been removed (duplicate code cleaning)');
        return false;
    },
     ...
     * console.log('Recipe default value:', defaultValue);
     */
    getRecipeDefault: function(recipeId) {
        console.log('§e[Shanhai Recipe API] getRecipeDefault function has been removed (duplicate code cleaning)');
        return null;
    },
     ...
     *     'recipe_c': false
     * });
     * console.log('Batch setting results:', result);
     */
    batchSetRecipeDefaults: function(defaults) {
        console.log('§e[Shanhai Recipe API] batchSetRecipeDefaults function has been removed (duplicate code cleaning)');
     ...
     * console.log('Total number of default values:', Object.keys(allDefaults).length);
     */
    getAllRecipeDefaults: function() {
        console.log('§e[Shanhai Recipe API] getAllRecipeDefaults function has been removed (duplicate code cleaning)');
        return {};
    },
    ...
    initializeMissingDefaults: function(defaultValue) {
        console.log('§e[Shanhai Formula API] initializeMissingDefaults function has been removed (duplicate code cleaning)');
     ...
     * console.log('Result of reset:', success);
     */
    resetRecipeLoadConfigToDefaults: function() {
        if (typeof global.shanhaiRecipeLoadConfig !== 'undefined') {
            global.shanhaiRecipeLoadConfig = {};
            console.log('§a[Shanhai Recipe API] Recipe loading configuration has been reset to default value');
            return true;
        }
        console.log('§e[Shanhai Recipe API] Recipe loading configuration is undefined, no need to reset');
        return false;
    },
    ...
    findRecipeInAllSources: function(recipeId) {
        ...
        if (global.shanhaiRecipeInfoCollector) {
            ...
            if (collected) {
                return { recipe: collected, source: 'Recipe Collector' };
            }
        }
        ...
        for (var i = 0; i < recipeArrays.length; i++) {
            ...
            if (arr.data && Array.isArray(arr.data)) {
                ...
                if (found) {
                    return { recipe: found, source: 'Recipe array:' + arr.name };
                }
            }
        }
    ...
    formatRecipeInfo: function(sender, result, recipeId) {
        ...
        sender.tell('§6═══════ Recipe information ═══════');
        ...
        if (recipe.defaultEnabled !== undefined) {
            sender.tell(`§7默认启用: ${recipe.defaultEnabled ? '§ais' : '§cNo'}`);
        }
        sender.tell('§6═══════════════════════');
    },
     ...
     * let gradient = global.shanhaiRecipeAPI.getTextUtilGradient("Generated by CellAPI, display generated by JEIcellAPI", "ultimateRainbow");
...
var DShanhaiSDA = {
    create: function(name) {
        var api = Java.loadClass('com.dishanhai.gt_shanhai.api.DShanhaiNBTAPI');
        return {
            _items: [], _vc: [], _name: name || 'super disk array', _lore: [],
...
ServerEvents.recipes(function(e) {
    ...
    function setLocalRecipeDefault(recipeId, defaultValue) {
        if (typeof recipeId !== 'string' || !recipeId.trim()) {
            warn('setLocalRecipeDefault: recipe ID must be a valid string');
            return false;
        }
        if (typeof defaultValue !== 'boolean') {
            warn('setLocalRecipeDefault: The default value must be a boolean (true/false)');
            return false;
        }
        localRecipeDefaults[recipeId] = defaultValue;
        debug('✅ Set recipe local default value:' + recipeId + ' = ' + defaultValue);
        return true;
    }
    ...
    function removeLocalRecipeDefault(recipeId) {
        if (localRecipeDefaults.hasOwnProperty(recipeId)) {
            ...
            debug('🗑️ Delete recipe local default value:' + recipeId);
            return true;
        }
        return false;
    }
    ...
    global.shanhaiAPI.clearLocalDefault = function(recipeId) {
        ...
        try {
            ...
            if (totalRemoved > 0) {
                debug('Cleared local defaults for recipes:' + recipeId + '(common' + totalRemoved + 'variations)');
            }
            return totalRemoved > 0;
        } catch (e) {
            error('Error clearing local defaults:' + e);
            return false;
        }
    };
    ...
    function generateRecipeStatistics() {
        ...
        if (totalRecipes === 0) {
            info('📊 Recipe statistics: No recipe information collected');
            return;
        }
    ...
    info('Recipe array exported to global object');
...
ServerEvents.recipes(e => {
    var timer_mek_remove = new Timer('Mekanism Recipe Remove Module');
    info('📝 Start processing Mekanism/Allthemodium recipe removal...');
    ...
    if (Platform.isLoaded('mekanism')){
        ...
        removeList.forEach(item => {
            try {
                e.remove(item);
                debug(`删除配方: input=${item.input}, mod=${item.mod || 'none'}`);
            } catch(err) {
        ...
        outputRemoveList.forEach(item => {
            try {
                e.remove(item);
                debug(`删除输出配方: output=${item.output}, mod=${item.mod || 'none'}`);
            } catch(err) {
...
ServerEvents.tags('item', e => {
    var timer_item_tags = new Timer('Item label modification');
    info('🏷️ Modify item label initialization...');
    ...
    try {
        ...
        debug('Tag modification completed');
    } catch(err) {
...
ServerEvents.tags('fluid', e => {
    var timer_fluid_tags = new Timer('Fluid label modification');
    info('💧 Start modifying fluid labels...');
...
ServerEvents.tags('item', event => {
    var timer_batch_item_tags = new Timer('Bulk item label deletion');
    info('🗑️ Start deleting item tags in batches...');
...
function parseItemStringCellAPI(str) {
    if (!str || typeof str !== 'string') {
        throw new Error('Invalid item string:' + str);
    }
    ...
    if (!match) {
        throw new Error("Invalid item format, should use 'Quantity x ItemID' or 'Quantity x ItemID@InternalID' or 'ItemID':" + str);
    }
...
function parseFluidStringCellAPI(str) {
    if (!str || typeof str !== 'string') {
        throw new Error('Invalid fluid string:' + str);
    }
    
    let match = str.match(/^(\d+)\s*(mb|mB|b|B)?\s*(.+)$/i);
    if (!match) {
        throw new Error("Invalid fluid format, should use 'quantity fluid_name' or 'quantity mb fluid_name':" + str);
    }
...
function validateCellRecipe(itemList, inputItems) {
    ...
    if (!itemList || !Array.isArray(itemList) || itemList.length === 0) {
        errors.push('Item list cannot be empty');
    }
...
function checkCompatibility(expectedVersion) {
    if (!expectedVersion) {
        throw new Error('expectedVersion parameter cannot be empty');
    }
...
function measurePerformance(fn, context) {
    return function() {
        ...
        try {
            ...
            if (duration > 1000) {
                warn('[256k Cell API] Performance warning:' + (fn.name || 'anonymous function') + 'Execution time' + duration + 'ms');
            }
            return result;
        } catch (err) {
            var duration = Date.now() - start;
            error('[256k Cell API] Performance error:' + (fn.name || 'anonymous function') + 'implement' + duration + 'Failed after ms:' + err.message);
            throw err;
        }
    };
}
...
function buildCellNBTCellAPI(items, cellName, lore) {
    try {
        ...
        error('[CellAPI] Java NBT build failed:' + e.message);
        return '';
    }
}
...
function addCellAssemblerRecipeCellAPI(recipeId, cellName, itemList, lore, inputItems, inputFluids, circuit, duration, eut, defaultEnabled) {
    ...
    try { 
        ...
        if (_registeredCellRecipes.has(recipeId)) {
            warn('[256k Cell API] Recipe' + recipeId + 'Already exists, skip registration');
            return false;
        }
        ...
        if (!recipeId || !recipeId.includes(':')) { 
            throw new Error('Recipe ID format is incorrect, should use namespace:path format'); 
        } 
        
        // ========== 默认值系统检查（继承配方默认值系统的启用/禁用逻辑） ==========
        if (!_isCellRecipeEnabled(recipeId, defaultEnabled)) {
            info('[256k Cell API] Recipe' + recipeId + 'Disabled by default system, skip registration');
        ...
        return function(gtceu) { 
            ...
            info('[256k Cell API] Recipe generated:' + recipeId); 
            info('Item pack:' + cellName + ' (' + itemList.length + 'items)'); 
            info('Voltage:' + eut + 'EU/t, time-consuming:' + duration + ' ticks'); 
            info('Circuit:' + circuit + ', enter the item:' + inputItems.length + 'kind');
            
            return true; 
        }; 
        
    } catch (err) { 
        error('[256k Cell API] Recipe generation failed (' + recipeId + '): ' + err.message); 
        return null; 
    } 
}
...
function getCellContentCellAPI(cellItem) {
    try {
        ...
        error('[256k Cell API] Failed to parse item pack content:' + err.message);
        return [];
    }
}
...
var CellAPI = {
    ...
    addBatchRecipes: function(recipes, gtceu) {
        if (!recipes || !Array.isArray(recipes)) {
            throw new Error('The recipes parameter must be an array');
        }
        if (!gtceu) {
            throw new Error('The gtceu parameter cannot be empty');
        }
        ...
        recipes.forEach(function(recipe) {
            try {
                ...
                error('[256k Cell API] Batch registration of recipes failed (' + (recipe.id || 'unknown') + '): ' + err.message);
                failed++;
            }
        });
        info('[CellAPI] Batch registration completed: Success' + success + ', fail' + failed);
    ...
    infinityCell: function(itemString, cellName, lore) {
        ...
        if (!itemString || typeof itemString !== 'string') {
            throw new Error('itemString parameter must be a string');
        }
        ...
        if (!parsed) {
            throw new Error('Unable to parse infinite cell format:' + itemString);
        }
        
        // 验证是否为无限单元格
        if (!parsed.id.includes('infinity_cell')) {
            warn('[CellAPI.infinityCell] Warning: The item ID does not contain "infinity_cell", but the format contains the @ symbol:' + itemString);
        }
        
        // 构建NBT标签
        var itemList = [itemString];
        var nbt = buildCellNBTCellAPI(itemList, cellName || 'infinite cells', lore || ['§6infinite cells', '§7Internal items:' + parsed.innerId]);
    ...
    addInfinityCellRecipe: function(recipeId, infinityCellString, inputItems, inputFluids, circuit, duration, eut, gtceu) {
        ...
        if (!infinityCellString || typeof infinityCellString !== 'string') {
            throw new Error('infinityCellString parameter must be a string');
        }
        ...
        if (!parsed) {
            throw new Error('Unable to parse infinite cell format:' + infinityCellString);
        }
        
        // 验证是否为无限单元格
        if (!parsed.id.includes('infinity_cell')) {
            warn('[CellAPI.addInfinityCellRecipe] Warning: The item ID does not contain "infinity_cell", but the format contains the @ symbol:' + infinityCellString);
        }
        
        // 生成单元格名称和描述
        var cellName = 'Infinite cells:' + (parsed.innerId || 'unknown');
        var lore = [
            '§6infinite cells',
            '§7Internal items:' + (parsed.innerId || 'unknown'),
            '§7Quantity: Unlimited'
        ];
        ...
        if (result) {
            info('[CellAPI.addInfinityCellRecipe] Infinite cell recipe registered:' + recipeId + ' (' + infinityCellString + ')');
        }
    ...
    // 版本信息
    version: '1.0.0',
    author: 'Mountains and seas are everlasting/dishanhai'
};

// 导出到全局
if (typeof global !== 'undefined') {
    global.CellAPI = CellAPI;
    info('[256k Cell API] Loaded, version' + CellAPI.version);
    
    // 检查依赖
    let deps = checkDependenciesCellAPI();
    if (!deps.allLoaded) {
        info('[256k Cell API] Missing dependencies:');
        if (!deps.ae2) info('- AE2 not loading');
        if (!deps.gtceu) info('- GTCEu not loading');
    } else {
        info('[256k Cell API] All dependencies are met');
    }
}
...
info('[256k Cell API] Number of hot reloads:' + global.__kubejs_cell_api_reload_count);
...
function getRecipeDetails(recipe) {
    if (!recipe) return 'No recipe information';
    let details = 'ID: ' + recipe.id + 'type:' + recipe.type + '\n';
    if (recipe.itemInputs) details += 'Item input:' + JSON.stringify(recipe.itemInputs) + '\n';
    if (recipe.inputFluids) details += 'Fluid input:' + JSON.stringify(recipe.inputFluids) + '\n';
    if (recipe.itemOutputs) details += 'Item output:' + JSON.stringify(recipe.itemOutputs) + '\n';
    if (recipe.outputFluids) details += 'Fluid output:' + JSON.stringify(recipe.outputFluids) + '\n';
    if (recipe.EUt !== undefined) details += 'EU/t: ' + recipe.EUt + '\n';
    if (recipe.duration !== undefined) details += 'time consuming:' + recipe.duration + '\n';
    if (recipe.circuit !== undefined) details += 'Circuit:' + recipe.circuit + '\n';
    if (recipe.notConsumable !== undefined) details += 'Non-consumables:' + recipe.notConsumable + '\n';
    return details;
}
...
function saveConfigToFile(config) {
    try {
        if (typeof JsonIO !== 'undefined' && typeof JsonIO.write === 'function') {
            ...
            console.log('§a[Configuration Repair] Configuration saved:' + Object.keys(config).length + 'items');
            return true;
        }
    } catch (err) {
        console.log('§c[Configuration Repair] Failed to save configuration:' + err.message);
    }
    return false;
}
...
ServerEvents.loaded(event => {
    ...
    (function() {
        ...
        function collectRecipeDefaultsFromCollector() {
            ...
            if (!collector || typeof collector !== 'object') {
                console.log('§e[Configuration Fix] Recipe collector does not exist');
                return null;
            }
            
            var totalKeys = Object.keys(collector).length;
            console.log('§7[Configuration Fix] Total collector keys:' + totalKeys);
            ...
            console.log('§a[Configuration fix] Obtained from collector' + count + 'recipe default value');
        ...
        function syncAllRecipesToConfig(forceOverwrite) {
            ...
            console.log('§6[Configuration Fix] Start synchronizing all recipes to configuration files...');
            
            if (typeof global !== 'undefined' && global.shanhaiRecipeConfigJustReset === true) {
                console.log('§e[Configuration Fix] Reset flag detected, sync skipped');
                return false;
            }
            
            var allDefaults = collectRecipeDefaultsFromCollector();
            if (!allDefaults || Object.keys(allDefaults).length === 0) {
                console.log('§e[Configuration Fix] Collector is empty and cannot be synchronized');
                return false;
            }
            ...
            for (var key in allDefaults) {
                if (allDefaults.hasOwnProperty(key)) {
                    if (finalConfig[key] === undefined) {
                        ...
                        addedCount++;
                        console.log('§7[Configuration Fix] Add new recipe:' + key + ' = ' + (allDefaults[key] ? 'enable' : 'Disable'));
                    } else {
                        skippedCount++;
                        // 已存在配置，保留用户设置，不覆盖
                    }
            ...
            console.log('§a[Configuration Fix] New' + addedCount + 'recipe, reserved' + skippedCount + 'individual user configuration');
            
            if (addedCount > 0) {
                saveConfigToFile(finalConfig);
            } else {
                console.log('§a[Configuration Repair] The configuration is up to date, total' + Object.keys(finalConfig).length + 'recipes');
            }
        ...
        function trySync(e) {
            attempts++;
            console.log('§7[CONFIGURATION FIX] Try synchronizing recipes (section' + attempts + '/' + maxAttempts + 'Second-rate)');
            
            var collector = global.shanhaiRecipeCollector || global.shanhaiRecipeInfoCollector;
            var collectorSize = collector ? Object.keys(collector).filter(function(k) { return k !== '_statistics'; }).length : 0;
            
            if (collectorSize > 0) {
                console.log('§a[Configuration Fix] Collector already exists' + collectorSize + 'recipes');
                ...
                console.log('§e[Configuration Fix] Maximum attempts reached, collector still empty');
            }
        }
        
        console.log('§6[Configuration Fix] Configuration persistence fix loaded');
    ...
    if (typeof recipes !== 'undefined') global.recipes = recipes;
    if (typeof recipes_electrolyzers !== 'undefined') global.recipes_electrolyzers = recipes_electrolyzers;
    info('Recipe array exported to global object (ServerEvents.loaded)');
    ...
    if (global.__shanhai_guard__) {
        ...
        if (global.shanhaiAPI) {
            ...
            info('§6[Mountain and Sea Protection Layer] §aThe main API has been sealed and protected §r');
        }
        if (global.shanhaiRecipeAPI) {
            guard.sealAPI(global.shanhaiRecipeAPI, 'shanhaiRecipeAPI');
            info('§6[Mountain and Sea Protective Layer] §aFormula API has been sealed and protected §r');
        }
        if (global.shanhaiRecipeControlAPI) {
            guard.sealAPI(global.shanhaiRecipeControlAPI, 'shanhaiRecipeControlAPI');
            info('§6[Mountain and Sea Protection Layer] §aFormula control API has been sealed and protected §r');
        }
    }
