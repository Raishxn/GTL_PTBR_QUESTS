...
ServerEvents.recipes(function(event) {
  // 诊断: 打印可用的山海全局绑定
  console.log('[Shan Hai Recipe Rules] global keys:', Object.keys(global).filter(k => k.toLowerCase().includes('shanhai') || k.toLowerCase().includes('recipe')))
  console.log('[Shanhai Recipes Rules] typeof ShanhaiRecipes:', typeof ShanhaiRecipes, 'typeof global.ShanhaiRecipes:', typeof global.ShanhaiRecipes)

  var R = global.ShanhaiRecipes || ShanhaiRecipes
  if (!R) {
    console.error('[Shanhai Recipes Rules] ShanhaiRecipes is not defined, skip all rules')
    return
  }
  ...
  // console.info('[山海] found' + ids.length + 'Nano Swarm Formula')

  console.info('[Shan Hai·Rule Management] Recipe rules have been loaded')
})
