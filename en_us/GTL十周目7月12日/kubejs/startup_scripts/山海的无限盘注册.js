...
StartupEvents.modifyCreativeTab("expatternprovider:tab_main", e => {
    ...
    function shanhai$buildGtceuAllFluidSDA() {
        ...
        return DShanhaiNBTAPI.buildSDAFromList(
            entries,
            'GTCEu fully fluid infinite array-de-flow version',
            ['§7Temporary re-export: only retain GTCEu source fluid', '§7has blocked flowing_* fluid ID', '§7Fluid type: §e' + entries.length],
            []
        );
    }
    ...
    try {
        ...
        console.error('[Shanhai SDA] GTCEu full fluid infinite array generation failed:' + err);
    }
