...
JEIEvents.addItems(event => {
    ...
    if (typeof global.kirin === 'undefined') {
        ...
        let nbtString = `{RepairCost:0,amts:[L;${amtsList}],display:{Name:'{"text":"§rinfinite programming circuit component package"}'},ic:${size}L,internalCurrentPower:20000.0d,keys:[${keysList.join(',')}]}`
...
ItemEvents.tooltip(event => {
    ...
    event.add('disksavior:lecb', '§b§oDear friends...')
    ...
    event.addAdvanced('disksavior:steam_1', (item, advanced, text) => {
        if (event.shift) {
            ...
            text.add('')
            text.add('§4§m§n§oThen why not use other power generation methods?')
        } else {
            text.add('§bis placed in a semi-fluid generator')
            text.add('§bWater instead of steam')
            text.add('§bprovides 32768 times overclocking')
            text.add('§7§oI am the bone of my steam')
            text.add('§7§oSteel is my body, and water is my blood')
            text.add('§aPress and hold §eSHIFT §r§aView advice')
        }
    })
    ...
    event.addAdvanced('disksavior:steam_2', (item, advanced, text) => {
        if (event.shift) {
            ...
            text.add('§bis placed in a semi-fluid generator')
            text.add('§bWater instead of steam')
            text.add('§bprovides 32768*256 times overclocking')
    ...
    event.addAdvanced('disksavior:steam_3', (item, advanced, text) => {
        if (event.shift) {
            ...
            text.add('Engraving proof of "I have been here"')
            text.add('')
            text.add('Just when he was casting the turbine')
            text.add('The words §4MSPT §ron his body are getting darker and darker.')
            ...
            text.add('§bis placed in a semi-fluid generator')
            text.add('§bWater instead of steam')
            text.add('§bprovides 32768*256^2 times overclocking')
    ...
    event.addAdvanced('disksavior:steam_4', (item, advanced, text) => {
        if (event.shift) {
            ...
            text.add('')
            text.add('When he wanted to jump from that forest')
            ...
            text.add('§bis placed in a semi-fluid generator')
            text.add('§bWater instead of steam')
            text.add('§bprovides 32768*256^3 times overclocking')
    ...
    event.addAdvanced('disksavior:steam_is_my_last_life', (item, advanced, text) => {
        if (event.shift) {
            if (event.ctrl && event.alt) {
                ...
                text.add('§4......')
                text.add('§cPress and hold §4SHIFT §r§c+ §r§4CTRL §r§c+ §r§4ALT §r§cView mental image')
            } else {
                text.add('He finally stood at the top of the steam')
                text.add('Underfoot are countless roaring semi-liquid generators')
                text.add('')
                ...
                text.add('Then he began to melt the remains')
                text.add('This is extremely painful')
                text.add('Every turbine is built by him himself.')
                ...
                text.add('Only by constantly burning one\'s own origin')
                text.add('Only by constantly forging new turbines')
                text.add('Until the metal in his hand became the title')
                text.add('§4§oUntil his heart melts away')
                text.add('§4......')
                text.add('§cHold §4SHIFT §r§c+ §r§4CTRL §r§cView chant')
            }
        } else {
            text.add('§cis placed in a semi-fluid generator')
            text.add('§cWater instead of steam')
            text.add('§cprovides §k2^57 §r§ctimes overclocking')
            text.add('§4......')
            text.add('§4§o§kUnlimited Steam Works')
            text.add('§cPress and hold §4SHIFT §r§cView advice')
        }
    })
})