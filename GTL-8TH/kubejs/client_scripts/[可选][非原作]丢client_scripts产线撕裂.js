...
JEIEvents.addItems(event => {
    console.log('Registering custom AE component package with JEI...')
    // --- 批量注册无限流体单元 ---
    ...
    {
        event.add(Item.of(parallel_hatch_list[i], `{BlockEntityTag:{currentParallel:${4**(i+4)}},display:{Lore:[\'"One-stage compression Actual number of parallels: ${4**(i+4)} Place carefully Parallelism will be reset after mining/clicking"\']}}`))
        event.add(Item.of(parallel_hatch_list[i], `{BlockEntityTag:{currentParallel:${4**(i+5)}},display:{Lore:[\'"Double compression Actual number of parallels: ${4**(i+5)} Place carefully Parallelism will be reset after mining/clicking"\']}}`))
        event.add(Item.of(parallel_hatch_list[i], `{BlockEntityTag:{currentParallel:${4**(i+6)}},display:{Lore:[\'"Triple compression Actual number of parallels: ${4**(i+6)} Place with caution Parallelism will be reset after mining/clicking"\']}}`))
    }//,display:{Lore:[\'""\']}//已解锁悖论实现理论的宏原子 塞了382个星阵 作为主机时配合永恒线圈获得 Long.MAX 并行
    event.add(Item.of('gtladditions:macro_atomic_resonant_fragment_stripper', '{BlockEntityTag:{astralArrayCount:382},display:{Lore:[\'"The macro atom of the paradox realization theory has been unlocked. 382 star arrays have been stuffed into it. When used as a host, cooperate with the eternal coil to obtain Long.MAX parallelism."\']}}'))
    event.add(Item.of('gtceu:configurable_maintenance_hatch', '{BlockEntityTag:{durationMultiplier:2.0f,maintenanceProblems:127b},display:{Lore:[\'"The time multiplier is 2.0x. It has been maintained once. Place it carefully. The maintenance status and time multiplier will be reset after mining."\']}}'))
    event.add(Item.of('gtceu:auto_configuration_maintenance_hatch', '{BlockEntityTag:{durationMultiplier:2.0f},display:{Lore:[\'"The time multiplier is 2.0x. Place it carefully. The time multiplier will be reset after mining."\']}}'))
    // --- 批量注册无限原木单元 ---
    ...
    // 究极宇宙探测
    event.add(packed_infinity_cell('Space exploration component package', 'f', ['gtceu:cosmic_element', 'gtceu:starlight', 'gtceu:heavy_lepton_mixture']))

    // 究极集气
    event.add(packed_infinity_cell('Gas collection component package', 'f', ['gtceu:air', 'gtceu:liquid_air', 'gtceu:nether_air', 'gtceu:liquid_nether_air', 'gtceu:ender_air', 'gtceu:liquid_ender_air']))

    // 究极木石化
    event.add(packed_infinity_cell('Petrified wood component package', 'f', ['gtceu:ethanol','gtceu:naphthalene', 'gtceu:octane', 'gtceu:ethane', 'gtceu:propane', 'gtceu:butane', 'gtceu:toluene', 'gtceu:benzene', 'gtceu:butadiene', 'gtceu:butene', 'gtceu:propene', 'gtceu:ethylene', 'gtceu:methanol', 'gtceu:absolute_ethanol', 'gtceu:methane', 'gtceu:methyl_acetate', 'gtceu:acetic_acid', 'gtceu:carbon', 'gtceu:creosote', 'gtceu:carbon_monoxide', 'gtceu:dimethylbenzene', 'gtceu:acetone', 'gtceu:phenol']))

    // 鸿蒙元件包
    event.add(packed_infinity_cell_if('Hongmeng component package', homoitem, homofluid))

    //脆弱工具元件包
    event.add(packed_infinity_cell('Fragile Tool Component Pack','i',['fragile_tool:fragile_file', 'fragile_tool:fragile_hammer', 'fragile_tool:fragile_mallet', 'fragile_tool:fragile_wrench', 'fragile_tool:fragile_wire_cutter', 'fragile_tool:fragile_crowbar', 'fragile_tool:fragile_saw', 'fragile_tool:fragile_screwdriver','fragile_tool:fragile_knife','fragile_tool:fragile_mortar']))

    //通用电路板元件包
    event.add(packed_infinity_cell('Universal circuit board component package','i',['kubejs:ulv_universal_circuit', 'kubejs:lv_universal_circuit', 'kubejs:mv_universal_circuit', 'kubejs:hv_universal_circuit', 'kubejs:ev_universal_circuit', 'kubejs:iv_universal_circuit', 'kubejs:luv_universal_circuit', 'kubejs:zpm_universal_circuit', 'kubejs:uv_universal_circuit', 'kubejs:uhv_universal_circuit', 'kubejs:uev_universal_circuit', 'kubejs:uiv_universal_circuit', 'kubejs:uxv_universal_circuit', 'kubejs:opv_universal_circuit', 'kubejs:max_universal_circuit']))
    ...
    if (Platform.isLoaded('gtl_extend')) {
        ...
        event.add(packed_infinity_cell_if('Black hole component package', homoitem, bhmdo))
    }
