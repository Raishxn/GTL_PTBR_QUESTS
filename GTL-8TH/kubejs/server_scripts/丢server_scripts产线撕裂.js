...
ServerEvents.recipes((event) => {
    ...
    for(let i = 0;i<parallel_hatch_list.length;i++)
    {
        gtr.compressor(`thetornproductionline:${parallel_hatch_list[i].split(':')[1]}_compress_1`)
            ...
            .itemOutputs(Item.of(parallel_hatch_list[i], `{BlockEntityTag:{currentParallel:${4**(i+4)}},display:{Lore:[\'"One-stage compression Actual number of parallels: ${4**(i+4)} Place carefully Parallelism will be reset after mining/clicking"\']}}`))
            .EUt(2**(i*2+13))
            .duration(500)
        gtr.assembler(`thetornproductionline:${parallel_hatch_list[i].split(':')[1]}_compress_2`)
            .inputs(GTItemof(parallel_hatch_list[i], `{BlockEntityTag:{currentParallel:${4**(i+4)}},display:{Lore:[\'"One-stage compression Actual number of parallels: ${4**(i+4)} Place carefully Parallelism will be reset after mining/clicking"\']}}`,8))
            .itemInputs('8x #gtceu:circuits/'+parallel_hatch_list[i].split(':')[1].split('_')[0])
            .itemOutputs(Item.of(parallel_hatch_list[i], `{BlockEntityTag:{currentParallel:${4**(i+5)}},display:{Lore:[\'"Double compression Actual number of parallels: ${4**(i+5)} Place carefully Parallelism will be reset after mining/clicking"\']}}`))
            .EUt(2**(i*2+13))
            .duration(500)
            .circuit(2)
        gtr.assembler(`thetornproductionline:${parallel_hatch_list[i].split(':')[1]}_compress_3`)
            .itemOutputs(Item.of(parallel_hatch_list[i], `{BlockEntityTag:{currentParallel:${4**(i+6)}},display:{Lore:[\'"Triple compression Actual number of parallels: ${4**(i+6)} Place with caution Parallelism will be reset after mining/clicking"\']}}`))
            .inputs(GTItemof(parallel_hatch_list[i], `{BlockEntityTag:{currentParallel:${4**(i+5)}},display:{Lore:[\'"Double compression Actual number of parallels: ${4**(i+5)} Place carefully Parallelism will be reset after mining/clicking"\']}}`,16))
    ...
    .itemOutputs(Item.of('gtceu:configurable_maintenance_hatch', "{BlockEntityTag:{durationMultiplier:2.0f,maintenanceProblems:127b},display:{Lore:['\"The time-consuming multiplier is 2.0x. It has been maintained once. Place it carefully. The maintenance status and time-consuming multiplier will be reset after mining\"']}}"))
    ...
    .itemOutputs(Item.of('gtceu:auto_configuration_maintenance_hatch', "{BlockEntityTag:{durationMultiplier:2.0f},display:{Lore:['\"The time-consuming multiplier is 2.0x. Place it carefully. The time-consuming multiplier will be reset after mining\"']}}"))
...
*/
   ...
   gtr.alloy_blast_smelter('thetornproductionline:easier_quantanium')//炖量子
    ...
    gtr.large_chemical_reactor('thetornproductionline:easier_osmium_tetroxide_dust')//四氧化锇单步
        ...
        .EUt(GTValues.VA[GTValues.LV])
        .duration(80)
    gtr.assembly_line('thetornproductionline:creative_computation_provider')//If you are tired of supercomputers'低'算力
