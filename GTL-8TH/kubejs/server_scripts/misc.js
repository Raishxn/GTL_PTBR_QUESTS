    ...
    if (!event.player.isFake() && event.player.isSteppingCarefully()) {
        ...
        if (recipeLogic != null && recipeLogic.isWorking()) {
            ...
            if (eu > 0 && $WirelessEnergyManager.addEUToGlobalEnergyMap(event.player.uuid, $BigInteger.valueOf(- eu), recipeLogic.getMachine())) {
                ...
                event.player.setStatusMessage("consumed" + $FormattingUtil.formatNumbers(eu) + "EU, resulting in reduced machine run time" + reducedDuration + " tick")
            }
        }
    }
})
...
function consume(player, hand) {
    ...
    if (tt > 0) {
        player.setStatusMessage("Number of uses:" + (tt - 1))
    } else {
...
ItemEvents.firstRightClicked("kubejs:warped_ender_pearl", event => {
    ...
    if (event.player.isSteppingCarefully()) {
        ...
        event.player.setStatusMessage("Coordinates have been saved to this slot")
    }
    if (!event.player.isSteppingCarefully()) {
...
PlayerEvents.tick(event => {
    if (event.player.age % 20 == 0) {
        ...
        } else if (event.player.persistentData.getInt("fly") == 1 && !event.player.isCreative() && !event.player.isSpectator()) {
        ...
        if (tt > 20) {
            if (tti) {
                event.player.setStatusMessage("Number of uses:" + 20)
            }
        } else {
            event.player.persistentData.putInt("tt", tt + 1)
            if (tti) {
                event.player.setStatusMessage("Number of uses:" + (tt + 1))
            }
        }
    }
})
...
function flyspeed(player, max) {
    if (player.isSteppingCarefully()) {
        ...
        player.setStatusMessage("Flight speed reset")
        ...
        if (speed < max) {
            ...
            player.setStatusMessage("Flight speed x" + speeda)
        } else {
            player.setStatusMessage("reach the limit")
        }
    }
}
...
NetworkEvents.dataReceived("global.nightvisionKey.consumeClick", event => {
    if (event.player.getArmorSlots().toString() == "[1 fermium_boots, 1 fermium_leggings, 1 fermium_chestplate, 1 fermium_helmet]" || event.player.getArmorSlots().toString() == "[1 magnetohydrodynamicallyconstrainedstarmatter_boots, 1 magnetohydrodynamicallyconstrainedstarmatter_leggings, 1 magnetohydrodynamicallyconstrainedstarmatter_chestplate, 1 magnetohydrodynamicallyconstrainedstarmatter_helmet]" || event.player.getArmorSlots().toString() == "[1 space_fermium_boots, 1 space_fermium_leggings, 1 space_fermium_chestplate, 1 space_fermium_helmet]") {
        if (event.player.persistentData.getBoolean("nv")) {
            ...
            event.player.setStatusMessage("Night vision off")
        } else {
            event.player.persistentData.putBoolean("nv", true)
            event.player.setStatusMessage("Night vision on")
        }
    }
})
...
NetworkEvents.dataReceived("global.vajraKey.consumeClick", event => {
    ...
    if (item == "gtceu:echoite_vajra") {
        ...
        if (speed < 100) {
            ...
            event.player.setStatusMessage("speed:" + (speed + value))
        } else if (speed < 1000) {
            item.getNbt().getCompound("GT.Tool").putFloat("ToolSpeed", speed + value * 10)
            event.player.setStatusMessage("speed:" + (speed + value * 10))
        } else {
            item.getNbt().getCompound("GT.Tool").putFloat("ToolSpeed", 10)
            event.player.setStatusMessage("Speed: 10")
        }
    }
})
...
ItemEvents.firstLeftClicked("gtceu:echoite_vajra", event => {
    if (event.player.isSteppingCarefully() && !event.player.isMiningBlock()) {
        event.item.getNbt().getCompound("GT.Behaviours").getByte("RelocateMinedBlocks") == 1 ? (event.item.getNbt().getCompound("GT.Behaviours").remove("RelocateMinedBlocks"), event.player.setStatusMessage("Magnetic Attraction: Off")) : (event.item.getNbt().getCompound("GT.Behaviours").putByte("RelocateMinedBlocks", 1), event.player.setStatusMessage("Magnetic Attraction: Open"))
    }
})
