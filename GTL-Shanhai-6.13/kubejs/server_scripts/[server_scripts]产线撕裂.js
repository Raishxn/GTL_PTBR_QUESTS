...
ServerEvents.recipes((event) => {
    ...
    for(let i = 0;i<parallel_hatch_list.length;i++)
    {
        gtr.compressor(`thetornproductionline:${parallel_hatch_list[i].split(':')[1]}_compress_1`)
            ...
            .itemOutputs(Item.of(parallel_hatch_list[i], `{BlockEntityTag:{currentParallel:${4**(i+4)}},display:{Lore:[\'"Compressão de um estágio. Número real de paralelos: ${4**(i+4)}. Coloque com cuidado. O paralelismo será redefinido após a mineração/clique."\']}}`))
            .EUt(2**(i*2+13))
            .duration(500)
        gtr.assembler(`thetornproductionline:${parallel_hatch_list[i].split(':')[1]}_compress_2`)
            .inputs(GTItemof(parallel_hatch_list[i], `{BlockEntityTag:{currentParallel:${4**(i+4)}},display:{Lore:[\'"Compressão de um estágio. Número real de paralelos: ${4**(i+4)}. Coloque com cuidado. O paralelismo será redefinido após a mineração/clique."\']}}`,8))
            .itemInputs('8x #gtceu:circuits/'+parallel_hatch_list[i].split(':')[1].split('_')[0])
            .itemOutputs(Item.of(parallel_hatch_list[i], `{BlockEntityTag:{currentParallel:${4**(i+5)}},display:{Lore:[\'"Compressão dupla. Número real de paralelos: ${4**(i+5)}. Coloque com cuidado. O paralelismo será redefinido após a mineração/clique."\']}}`))
            .EUt(2**(i*2+13))
            .duration(500)
            .circuit(2)
        gtr.assembler(`thetornproductionline:${parallel_hatch_list[i].split(':')[1]}_compress_3`)
            .itemOutputs(Item.of(parallel_hatch_list[i], `{BlockEntityTag:{currentParallel:${4**(i+6)}},display:{Lore:[\'"Compressão tripla Número real de paralelos: ${4**(i+6)} Coloque com cuidado O paralelismo será redefinido após a mineração/clique"\']}}`))
            .inputs(GTItemof(parallel_hatch_list[i], `{BlockEntityTag:{currentParallel:${4**(i+5)}},display:{Lore:[\'"Compressão dupla. Número real de paralelos: ${4**(i+5)}. Coloque com cuidado. O paralelismo será redefinido após a mineração/clique."\']}}`,16))
    ...
    .itemOutputs(Item.of('gtceu:configurable_maintenance_hatch', "{BlockEntityTag:{ durationMultiplier:2.0f, maintenanceProblems:127b },display:{Lore:['\"O multiplicador demorado é 2,0x. Ele foi mantido uma vez. Coloque-o com cuidado. O status de manutenção e o multiplicador demorado serão redefinidos após a mineração\"']}}"))
...
*/
   ...
   gtr.alloy_blast_smelter('thetornproductionline:easier_quantanium')//炖量子
    ...
    const homoitem=["gtceu:carbon_dust",
        ...
        .circuit(11)
        .inputs(GTItemof_notConsumable('ae2:item_storage_cell_1k',packed_item_cell_nbt(packed_item_cell_items[4])))
        ...
        .EUt(4000000000000000000)
            ...
            .circuit(11)
            .inputs(GTItemof_notConsumable('ae2:item_storage_cell_1k',packed_item_cell_nbt(packed_item_cell_items[5])))
            .itemOutputs(homoitemoutputs2)
            .outputFluids(homofluidoutputs2)
            .duration(300)
    ...
    gtr.assembly_line('thetornproductionline:infinity_plant_ball')//无限植物球
        ...
        .itemOutputs(Item.of('e xpatternprovider:infinity_cell', '{record:{"#c":"ae2:i",id:"gtceu:plant_ball"}}'))
        .EUt(GTValues.VA[GTValues.UHV])
        .duration(4000)
