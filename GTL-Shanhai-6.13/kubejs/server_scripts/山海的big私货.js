...
global.shanhaiRecipeAPI = {
     ...
     * @returns {string} 彩色文本
     * @example
     * let rainbow = global.shanhaiRecipeAPI.getRandomRainbowText("Bens privados de montanha e mar");
     ...
     * @returns {string} 彩色文本
     * @example
     * let staticRandom = global.shanhaiRecipeAPI.getStaticRandomText("Bens privados de montanha e mar", "myseed");
     ...
     * @returns {string} 彩色文本
     * @example
     * let sessionRandom = global.shanhaiRecipeAPI.getSessionRandomSingleColorText("Bens privados de montanha e mar");
     ...
     * @returns {string} 彩色文本
     * @example
     * let fixed = global.shanhaiRecipeAPI.getFixedColorText("Bens privados de montanha e mar", "§c");
     ...
     * @returns {string} 彩色文本
     * @example
     * let alternating = global.shanhaiRecipeAPI.getAlternatingColorText("Bens privados de montanha e mar", "§c", "§9");
    ...
    getSummary: function() {
        ...
        var summary = "Estatísticas de fórmula de produto privado Shanhai";
        summary += "total:" + stats.total + "receitas";
        summary += "√Sucesso:" + stats.success + "individual";
        summary += "×Falha:" + stats.failed + "individual";
        
        if (stats.errors.length > 0) {
            summary += "Aviso: Feedback de erro da biblioteca de receitas, entre em contato com qq: 1982932217";//自行替换
            summary += "A versão atual dos bens privados de Deus:" + Version + "\n";
            summary += "Exemplo de falha X:";
            ...
            if (stats.errors.length > 5) {
                summary += "..além do mais" + (stats.errors.length - 5) + "erros";
            }
            
            summary += "Algumas receitas falharam ao carregar, notifique o administrador do servidor para verificar os logs";
        }
     ...
     * @returns {string} 彩色文本
     * @example
     * let rainbow = global.shanhaiRecipeAPI.getRainbowText("Bens privados de montanha e mar");
     ...
     * @returns {string} 渐变文本
     * @example
     * let gradient = global.shanhaiRecipeAPI.getGradientText("Bens privados de montanha e mar", "#FF0000", "#0000FF");
     ...
     * @example
     * // 创建动态颜色文本
     * let dynamicText = global.shanhaiRecipeAPI.createDynamicText("texto dinâmico");
     * 
     * // 创建彩虹文本
     * let rainbowText = global.shanhaiRecipeAPI.createDynamicText("texto arco-íris", { mode: 'rainbow' });
     * 
     * // 创建渐变文本
     * let gradientText = global.shanhaiRecipeAPI.createDynamicText("texto gradiente", { 
     *     mode: 'gradient', 
     *     startColor: '#FF0000', 
     *     endColor: '#0000FF' 
     * });
     ...
     * let gradient = global.shanhaiRecipeAPI.getTextUtilGradient("Gerado por CellAPI, display gerado por JEIcellAPI", "ultimateRainbow");
...
ServerEvents.recipes(function(e) {
    ...
    function _applyRecipeFields(machine, r) {
        if (r.triggerJsError) throw new Error("Erro de execução de teste de JavaScript: este é o erro gerado dentro da função de receita");
    ...
    function safeAddRecipe(arg1,arg2,arg3,arg4){
        ...
        // ========== 新增：处理 arg2 是配方对象的情况 ==========
            ...
            error("❌ O método de chamada safeAddRecipe está errado");
            broadcastRecipeError("safeAddRecipe", "invalid_parameters", "Método de chamada errado");
            return false;
        }
        ...
        if (!recipeEnabled) {
            ...
            return true; // 返回true表示"ignorado com sucesso"，不视为失败
        }
        ...
        if (!isGtRecipe && !isVanillaRecipe) {
            recordRecipe(type, false, id, "Máquina ou tipo de receita desconhecido");
            broadcastRecipeError(type, id, "Máquina ou tipo de receita desconhecido");
            return false;
        }
        ...
        try{
            ...
            if (typeof recipeFunc === 'function') {
                ...
                if (!gtr[type]) {
                    throw new Error("Tipo de máquina desconhecido:" + type);
                }
...
const universalRecipes = [
    ...
    // ========== 缺失流体产出配方 ==========
    { id: 'liquid_ending', type: 'chemical_reactor', inputFluids: ['gtceu:dragon_breath 1000', 'gtceu:ender_air 1000'], outputFluids: ['dishanhai:liquid_ending 2000'], EUt: hv, duration: 100 },
    { id: 'matter_fluid_basic', type: 'chemical_reactor', itemInputs: ['minecraft:redstone'], inputFluids: ['gtceu:distilled_water 1000'], notConsumable: 'dishanhai:wzcz1', outputFluids: ['dishanhai:matter_fluid_basic 1000'], EUt: mv, duration: 80 },
    { id: 'matter_fluid_advanced', type: 'chemical_reactor', itemInputs: ['gtceu:naquadah_dust'], inputFluids: ['dishanhai:matter_fluid_basic 4000', 'gtceu:enriched_naquadah 1000'], outputFluids: ['dishanhai:matter_fluid_advanced 1000'], EUt: iv, duration: 150 },
    { id: 'matter_fluid_ultimate', type: 'chemical_reactor', itemInputs: ['gtceu:spacetime_dust'], inputFluids: ['dishanhai:matter_fluid_transition 2000', 'gtceu:magmatter 1000'], outputFluids: ['dishanhai:matter_fluid_ultimate 1000'], EUt: uhv, duration: 200 },
    { id: 'matter_fluid_creative', type: 'chemical_reactor', itemInputs: ['gtceu:infinity_dust'], inputFluids: ['dishanhai:matter_fluid_ultimate 2000', 'gtceu:cosmic_computing_mixture 1000'], outputFluids: ['dishanhai:matter_fluid_creative 1000'], EUt: uev, duration: 400 },
];
...
let myRecipes = [
  ...
  { id: 'primordial_matter_recombination_wl_board_lv', type: 'primordial_matter_recombination', notConsumable: [], itemInputs: ["dishanhai:first_light"],inputFluids: ['dishanhai:light 2000'], outputFluids: ['dishanhai:matter_fluid_entry','dishanhai:zero_point_energy'], EUt: ulv, duration: 20 },
  // ===== LV 级 (EUt: 32) =====
  ...
  { id: 'matter_flow_condensation_basic', type: 'matter_flow_condensation', itemInputs: ['1x minecraft:ender_pearl'], inputFluids: ['gtceu:glass 1000', 'gtceu:polyethylene 1000'], outputFluids: ['dishanhai:matter_fluid_basic 1000'], EUt: 32, duration: 100, conditions: ["1x dishanhai:wzrm"] },
  // ===== MV 级 (EUt: 128) =====
  ...
  { id: 'matter_flow_condensation_foundation', type: 'matter_flow_condensation', itemInputs: ['1x gtceu:stainless_steel_ingot', '1x gtceu:silicon_ingot', '1x gtceu:exquisite_emerald_gem'], inputFluids: ['gtceu:kanthal 1000', 'gtceu:vanadium_steel 1000'], outputFluids: ['dishanhai:matter_fluid_foundation 1000'], EUt: 128, duration: 100, conditions: ["1x dishanhai:wzjc"] },
  ...
  { id: 'matter_flow_condensation_virtual', type: 'matter_flow_condensation', itemInputs: ['1x gtceu:nichrome_ingot', '1x gtceu:titanium_ingot', '1x minecraft:ender_eye'], inputFluids: ['gtceu:soldering_alloy 1000', 'gtceu:polytetrafluoroethylene 1000'], outputFluids: ['dishanhai:matter_fluid_virtual 1000'], EUt: 512, duration: 100, conditions: ["1x dishanhai:wzcz1"] },
  ...
  { id: 'matter_flow_condensation_transmutation', type: 'matter_flow_condensation', itemInputs: ['1x minecraft:nether_star', '1x gtceu:uranium_triplatinum_ingot', '1x gtceu:tungsten_steel_ingot'], inputFluids: ['gtceu:radon 1000', 'gtceu:epoxy 1000'], outputFluids: ['dishanhai:matter_fluid_transmutation 1000'], EUt: 2048, duration: 100, conditions: ["1x dishanhai:wzxc"] },
  ...
  { id: 'matter_flow_condensation_darkstar', type: 'matter_flow_condensation', itemInputs: [ '1x gtceu:rhodium_plated_palladium_ingot', '1x gtceu:samarium_iron_arsenic_oxide_ingot', '1x gtceu:hsse_ingot' ], inputFluids: ['gtceu:polybenzimidazole 1000', 'gtceu:styrene_butadiene_rubber 1000'], outputFluids: ['dishanhai:matter_fluid_darkstar 1000'], EUt: 8192, duration: 100, conditions: ["1x dishanhai:wzsb"] },
  ...
  { id: 'matter_flow_condensation_advanced_luv', type: 'matter_flow_condensation', itemInputs: [ '1x gtceu:naquadah_alloy_ingot', '1x gtceu:uranium_rhodium_dinaquadide_ingot', '1x gtceu:indium_tin_barium_titanium_cuprate_ingot' ], inputFluids: ['gtceu:duranium 1000', 'gtceu:europium 1000'], outputFluids: ['dishanhai:matter_fluid_advanced 1000'], EUt: 32768, duration: 100, conditions: ["1x dishanhai:wzax"] },
  ...
  { id: 'matter_flow_condensation_transition', type: 'matter_flow_condensation', itemInputs: [ '1x gtceu:tritanium_ingot', '1x gtceu:naquadria_ingot', '1x gtceu:enriched_naquadah_trinium_europium_duranide_ingot' ], inputFluids: ['gtceu:curium 1000', 'gtceu:antineutron 1000'], outputFluids: ['dishanhai:matter_fluid_transition 1000'], EUt: 131072, duration: 100, conditions: ["1x dishanhai:wzcz2"] },
  ...
  { id: 'matter_flow_condensation_zero', type: 'matter_flow_condensation', itemInputs: [ '1x gtceu:highurabilityompoundteel_ingot', '1x gtceu:pikyonium_ingot', '1x gtceu:ruthenium_trinium_americium_neutronate_ingot' ], inputFluids: ['gtceu:mutated_living_solder 1000', 'gtceu:antimatter 100'], outputFluids: ['dishanhai:matter_fluid_zero 1000'], EUt: uv, duration: 100, conditions: ["1x dishanhai:wzqs"] },
  ...
  { id: 'matter_flow_condensation_peak', type: 'matter_flow_condensation', itemInputs: [ '1x gtceu:dubnium_ingot', '1x gtceu:seaborgium_ingot', '1x gtceu:carbon_nanotubes_ingot' ], inputFluids: ['gtceu:enderium_plasma 1000', 'gtceu:cycloparaphenylene 1000'], outputFluids: ['dishanhai:matter_fluid_peak 1000'], EUt: uhv, duration: 100, conditions: ["1x dishanhai:wzgl"] },
  ...
  { id: 'matter_flow_condensation_ascension', type: 'matter_flow_condensation', itemInputs: [ '1x gtceu:taranium_ingot', '1x gtceu:attuned_tengam_ingot', '1x gtceu:echoite_ingot' ], inputFluids: ['gtceu:euv_photoresist 1000', 'gtceu:californium 1000'], outputFluids: ['dishanhai:matter_fluid_ascension 1000'], EUt: uev, duration: 100, conditions: ["1x dishanhai:wzhy"] },
  ...
  // ===== UIV 级 (EUt: uiv) =====
  { id: 'matter_flow_condensation_transcend', type: 'matter_flow_condensation', itemInputs: [ '1x gtceu:heavy_quark_degenerate_matter_ingot', '1x gtceu:legendarium_ingot', '1x gtceu:superheavy_h_alloy_ingot' ], inputFluids: ['gtceu:quark_gluon_plasma 1000', 'gtceu:heavy_lepton_mixture 1000'], outputFluids: ['dishanhai:matter_fluid_transcend 1000'], EUt: uiv, duration: 100, conditions: ["1x dishanhai:wzsw"] },
  ...
  // ===== UXV 级 (EUt: uxv) =====
  { id: 'matter_flow_condensation_chaos', type: 'matter_flow_condensation', itemInputs: [ '1x gtceu:crystalmatrix_ingot', '1x gtceu:starmetal_ingot', '1x gtceu:draconiumawakened_ingot' ], inputFluids: ['gtceu:radox 1000', 'gtceu:liquid_starlight 1000'], outputFluids: ['dishanhai:primal_chaos 1000'], EUt: uxv, duration: 100, conditions: ["1x dishanhai:wzcx"] },
  ...
  { id: 'matter_flow_condensation_eternal', type: 'matter_flow_condensation', itemInputs: [ '1x gtceu:spacetime_ingot', '1x gtceu:magmatter_ingot', '1x gtceu:transcendentmetal_ingot' ], inputFluids: ['gtceu:spacetime 1000', 'gtceu:primordialmatter 1000'], outputFluids: ['dishanhai:matter_fluid_eternal 1000'], EUt: opv, duration: 100, conditions: ["1x dishanhai:wzdf"] },
  ...
  // ===== MAX 级 (EUt: MAX) =====
  { id: 'matter_flow_condensation_ultimate', type: 'matter_flow_condensation', itemInputs: [ '1x gtladditions:creon_ingot', '1x gtladditions:mellion_dust', '1x gtladditions:super_dense_magmatter_plate' ], inputFluids: ['gtladditions:star_gate_crystal_slurry 1000', 'gtceu:spatialfluid 1000'], outputFluids: ['dishanhai:matter_fluid_ultimate 1000'], EUt: MAX, duration: 100, conditions: ["1x dishanhai:wzyh"] },
  ...
  { id: 'matter_module_casting_create_mk', type: 'matter_module_casting', itemInputs: [ '1x dishanhai:reality_anchor_module', '1x dishanhai:reality_core', '1x dishanhai:annihilation_core', '1x dishanhai:universal_parallel_overdriver', '1x gtladditions:apocalyptic_torsion_quantum_matrix', '1x gt_shanhai:spacetime_wave_matrix', '1x dishanhai:halo_end', '1x dishanhai:collapse_tear', '1x dishanhai:bridge_and_gate', '1x dishanhai:gate_and_bridg', '1x dishanhai:csj', '1x dishanhai:big_tear', '32x dishanhai:wl_board_eternal', '4x dishanhai:singularity_ring', '16x dishanhai:finality_certificate', '64x dishanhai:blue_son' ], inputFluids: [ 'dishanhai:matter_fluid_ultimate 1024000', 'dishanhai:stabilized_eternity 512000', 'dishanhai:primal_chaos 512000', 'dishanhai:causal_essence 512000' ], itemOutputs: ['1x dishanhai:create_mk'], EUt: MAX, duration: 200, conditions: ["64x dishanhai:wzyh"] },
  { id: 'matter_forging_spacetime_wave_matrix', type: 'matter_forging', itemInputs: ['64x gtceu:spacetime_ingot', '4x dishanhai:singularity_ring'], inputFluids: ['dishanhai:matter_fluid_ultimate 512000', 'gtceu:spacetime 512000'], itemOutputs: ['1x gt_shanhai:spacetime_wave_matrix'], EUt: MAX, duration: 200, conditions: ["64x dishanhai:wzyh"] },
  ...
  { id: 'matter_forging_central_finite_curve', type: 'matter_forging', itemInputs: [ '1x dishanhai:create_mk', '1x dishanhai:cshx', '1x dishanhai:primordial_engine_core', '1x dishanhai:reality_core' ], inputFluids: ['dishanhai:matter_fluid_ultimate 1024000', 'dishanhai:causal_essence 1024000'], itemOutputs: ['1x dishanhai:central_finite_curve'], EUt: MAX, duration: 400, conditions: ["64x dishanhai:wzcz3"] }
];
...
if (Platform.isLoaded('gt_shanhai')) {
    ...
    safeAddRecipe('spacetime_distortion', 'gt_shanhai:quantized_reality_fluid', () => {
        gtr.spacetime_distortion('gt_shanhai:quantized_reality_fluid')
            .itemInputs(
                'gtladditions:forge_of_the_antichrist',
                'dishanhai:annihilation_core'
        )
            .inputFluids(
                'gtceu:spacetime 50000',
                'gtladditions:star_gate_crystal_slurry 10000'
            )
            .outputFluids(
                'gtceu:spacetime 100000'
            )
            .EUt(ZPM)
            .duration(6000)
    })
    ...
    gtr.primordial_power_generator('zero_point_energy')
        ...
        .outputFluids('dishanhai:zero_point_energy 50')
        .duration(200)
    })
...
function parseItemStringCellAPI(str) {
    ...
    if (!match) {
        throw new Error("Formato de item inválido, deve usar 'Quantidade x ItemID' ou 'Quantidade x ItemID@InternalID' ou 'ItemID':" + str);
    }
...
function parseFluidStringCellAPI(str) {
    ...
    if (!match) {
        throw new Error("Formato de fluido inválido, deve usar 'quantity fluid_name' ou 'quantity mb fluid_name':" + str);
    }
...
function buildCellNBTCellAPI(items, cellName, lore) {
    ...
    var keysNBT = parsed.map(function(item) {
        ...
        tagPart = global._getItemTag ? global._getItemTag(item.id, item.innerId) : '';
...
function addCellAssemblerRecipeDirect(recipeId, cellName, itemList, lore, inputItems, inputFluids, circuit, duration, eut, defaultEnabled, gtceu) {
    ...
    try { 
        ...
        // ========== 默认值系统检查（继承 safeAddRecipe 的配方启用/禁用逻辑） ==========
        if (!_isCellRecipeEnabled(recipeId, defaultEnabled)) {
            info("Receita [API de célula 256k]" + recipeId + "Desativado por padrão do sistema, pule o registro");
            return false;
        }
...
var packed_cell_nbt2 = function(list, displayName, lore) {
    try { return Java.loadClass('com.dishanhai.gt_shanhai.api.DShanhaiNBTAPI').buildAECellNBT(list, displayName || null, displayName ? (Array.isArray(lore) ? lore : [lore]) : null); } catch(e) { return ''; }
};
...
ServerEvents.recipes(event => {
    ...
    try {
        ...
        var superAENBT = global.shanhaiPackDefs && global.shanhaiPackDefs['superAE'] ? global.shanhaiPackDefs['superAE'].nbt : '';
        event.shapeless(
            Item.of('ae2:portable_item_cell_256k', superAENBT),
            ['ae2:fluix_axe']
...
// ========== 无限染料元件包配方 ==========
    ...
    try {
        ...
        // 预处理：将染料 ID 转为 "1x infinity_cell@id" 格式，匹配全局构建器
        var _dyeItems = [];
        ...
        gtr.assembler(dyeRecipeId)
            ...
            .itemOutputs(Item.of('ae2:portable_item_cell_256k', DShanhaiNBTAPI.buildAECellNBTFromList(_dyeItems, '无限染料元件包pro', [
                '§7包含所有染料物品的无限元件包',
...
'1x gtladditions:space_infinity_integrated_ore_processor','426x gtlcore:power_module_7','6364x gtlcore:space_elevator_support','354x gtlcore:iridium_casing','2020x gtlcore:space_elevator_mechanical_casing','2x gtceu:infinity_frame','788x kubejs:space_elevator_internal_support','7347x kubejs:high_strength_concrete','1x kubejs:dimensional_bridge_casing','1x expatternprovider:infinity_cell@gtceu:stellar_energy_rocket_fuel'
      ...
      let  templateLore = [
        ...
        gtr.assembler(piggrecipeId)
            ...
            .itemOutputs(
            Item.of('ae2:portable_item_cell_256k', DShanhaiPackRegistry.get('skyBase') ? DShanhaiPackRegistry.get('skyBase').nbt() : '')
            )
...
try {
    ...
    let Machine = 10;
    let color_API_text = global.shanhaiDynamicLoreText || "§7é gerado por CellAPI e a exibição é gerada por JEIcellAPI";
    let templateLore = [
        '§7这是一个猪咪大礼包,猪咪大王的馈赠',
        '§7它只被授予给猪咪们,所以你是猪咪吗😋',
...
ServerEvents.recipes(e => {
    ...
    let [ulv, lv, mv, hv, ev, iv, luv, zpm, uv, uhv, uev, uiv, uxv, opv, max] = GTValues;    var ULV=GTValues[0],LV=GTValues[1],MV=GTValues[2],HV=GTValues[3],EV=GTValues[4],IV=GTValues[5],LuV=GTValues[6],ZPM=GTValues[7],UV=GTValues[8],UHV=GTValues[9],UEV=GTValues[10],UIV=GTValues[11],UXV=GTValues[12],OpV=GTValues[13],MAX=GTValues[14];
...
PlayerEvents.loggedIn(event => {
    ...
    event.server.scheduleInTicks(160, () => {
        ...
        if (typeof global.shanhaiRecipeStats !== 'undefined' && 
            ...
            // 发送统计信息给玩家
            player.tell(Component.gold("§m============= §lShanhai Estatísticas de Fórmula de Produto Privado §m=============="));
            
            if (total === 0) {
                player.tell(Component.yellow("§e⚠ As estatísticas da receita estão vazias e podem estar carregando de forma anormal"));
                player.tell(Component.yellow("§e💡 Verifique o log do servidor"));
                player.tell(Component.yellow("§e💽 É normal que a primeira receita de inicialização esteja vazia. Se não for a primeira inicialização, verifique o log do servidor."));
            } else if (failed === 0) {
                ...
                if (stats.errors && stats.errors.length > 0) {
                    player.tell(Component.red("§c❌ Detalhes da falha:"));
            ...
            if (failed > 0) {
                player.tell(Component.red("§c⚠ Falha ao carregar algumas receitas, a mensagem de erro específica é mostrada acima"));
                player.tell(Component.red('§c⚠ 日志路径:logs-kubejs-xxxxx.log'))
            }
        } else {
            player.tell(Component.yellow("§e⏳ As estatísticas da fórmula do produto privado Shanhai estão carregando, tente novamente mais tarde."));
            player.tell(Component.yellow("§e💡 Você também pode inserir §a/shanhai stats §epara consultar estatísticas de receita"));
        }
    });
});
