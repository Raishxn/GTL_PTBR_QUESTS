...
e.addAdvanced('dishanhai:create_mk', function(item, _, text) {
    if (e.shift) {
        addLore(text, [
            ...
            { text: '我们已然超越了"eterno"，枷锁已然碎裂。', bodyStyle: 'silver' },
...
e.addAdvanced('dishanhai:csj', function(item, advanced, text) {
    if (e.shift) {
        addLore(text, [
            ...
            { text: '他们在"ponte"的另一端，正不断地阻挠着对冲宇宙计划。', bodyStyle: 'crimson' },
...
e.addAdvanced('dishanhai:time_reversal_protocol', function(item, advanced, text) {
    if (e.shift) {
        addLore(text, [
            { text: '蓝星文明技术资料存档', style: 'ultimateRainbow' },
            { inlineText: '{body_silver}编号：甲■ ｜ 项目名：{/}{golden}【世线信标】{/}{body_silver} ｜ 第一序列{/}' },
            { inlineText: '{body_silver}项目解密序列：■■■■第一序列。根据协议，以下内容为时间回溯后解锁。{/}' },
            { inlineText: '{body_nature}-- 解锁成功 --{/}  {water}-- 相位力场发生器关闭 --{/}  {fire}-- 模因认知危害关闭 --{/}' },
            { text: '时间循环灾害末期被发现，经过多次测试，', bodyStyle: 'silver' },
            { text: '已证实可将指定物体置入时间循环当中。', bodyStyle: 'silver' },
            { text: '指定者质量上限未知、体积上限未知。', bodyStyle: 'silver' },
            { text: '已尝试过将整个恒星系进行回溯。', style: 'crimson' },
            { text: '为防止信息泄露，编号LH的恒星系已被处理。', bodyStyle: 'silver' },
            { text: '在物质世界中呈现为稳定单侧曲面状态', bodyStyle: 'silver' },
            { inlineText: '{body_silver}（俗称{/}{water}莫比乌斯环{/}{body_silver}），半径24.125米内的物质进入时间循环。{/}' },
            { text: '经过时空扰动实验室反复尝试，使用暗物质包裹后方可移动操控。', bodyStyle: 'silver' },
            { text: '凝聚计划与方舟工程准备完毕后，信标转移至方舟内部，', bodyStyle: 'silver' },
            { text: '指定方舟为时间循环物，以防宇宙战争中的意外情况。', bodyStyle: 'silver' },
            { text: '蓝星时空管理局 · 001号幕僚记录', style: 'golden' },
        ]);
    } else {
...
JEIEvents.addItems(function(event) {
    try {
        ...
        for (var sid in global.shanhaiPackDefs) {
            ...
            try {
                event.add(Item.of('ae2:portable_item_cell_256k', pd.nbt));
    ...
    try {
        ...
        if (dl && dl.length > 0) {
            ...
            try { event.add(Item.of('ae2:portable_item_cell_256k', DShanhaiNBTAPI.buildAECellNBTFromList(di, '无限染料元件包pro', ['§7包含所有染料物品的无限元件包','§7染料种类: §e' + dl.length + '§7 种','§7每个染料存储在无限元件包中','§8山海私货 v2.2']))); } catch(e) {}
        }
...
global.shanhaiColorAPI.isTextUtilAvailable = function() {
    ...
    if (typeof global.CellAPI !== 'undefined' && typeof global.CellAPI.addCellDescription === 'function') {
        ...
        if (typeof global.shanhaiColorAPI !== 'undefined' && typeof global.shanhaiColorAPI.getTextUtilGradient === 'function') {
            ...
            descriptionText = global.shanhaiColorAPI.getTextUtilGradient("Gerado por CellAPI, a exibição JEI é gerada por JEICellAPI", "ultimateRainbow");
            // 调试：检查返回值的类型
            ...
            if (typeof descriptionText !== 'string') {
                ...
                if (typeof descriptionText === 'function') {
                    ...
                    try {
                        ...
                        descriptionText = "Gerado por CellAPI, a exibição JEI é gerada por JEICellAPI";
                    }
                } else {
                    // 其他类型直接转换为字符串
            ...
            descriptionText = global.shanhaiDynamicLoreText || "Gerado por CellAPI, a exibição JEI é gerada por JEICellAPI";
        }
...
function addLore(textList, lines) {
    ...
    // 原初装配线模块
    ItemEvents.tooltip(function(e) {
        e.addAdvanced('gt_shanhai:primordial_assembly_line_module', function(item, _, text) {
            if (e.alt) {
                addLore(text, [
                { text: '§7┌─ 提升物品 ─────────────────────────────┐' },
                    { text: '§7│ §f无物品              → §7并行: §f64          §7│' },
                    { text: '§7│ §a入门物质模块        → §7并行: §b128         §7│' },
                    { text: '§7│ §a基础物质模块        → §7并行: §b256         §7│' },
                    { text: '§7│ §a物质推演模块      → §7并行: §b512         §7│' },
                    { text: '§7│ §a虚像物质模块            → §7并行: §b1,024       §7│' },
                    { text: '§7│ §a嬗变物质模块        → §7并行: §b1,024       §7│' },
                    { text: '§7│ §a暗星物质模块            → §7并行: §b4,096       §7│' },
                    { text: '§7│ §a物质重组模块      → §7并行: §b16,384      §7│' },
                    { text: '§7│ §a虚数物质模块        → §7并行: §b65,536      §7│' },
                    { text: '§7│ §a归零物质模块        → §7并行: §b524,288     §7│' },
                    { text: '§7│ §a巅峰物质模块            → §7并行: §b1,048,576   §7│' },
                    { text: '§7│ §a升维物质模块        → §7并行: §b2,097,152   §7│' },
                    { text: '§7│ §a超限物质模块        → §7并行: §b268,435,456 §7│' },
                    { text: '§7│ §a混沌物质模块            → §7并行: §b536,870,912 §7│' },
                    { text: '§7│ §a永恒物质模块        → §7并行: §b2,147,483,647§7│' },
                    { text: '§7│ §a物质创造模块      → §7并行: §d4.6e18      §7│' },
                    { text: '§7│ §5现实锚点模块      → §7并行: §d6.9e18      §7│' },
                    { text: '§7│ §a创始现实修改模块    → §7并行: §6§l无限      §7│' },
                    { text: '§7│ §5世线残片系列 → 线程倍率槽 §7│' },
                    { text: '§7│ §6初醒×1/共鸣×4/跃迁×16/超越×64         §7│' },
                    { text: '§7│ §6统合×256/归一×1024/裁决×4096(可堆叠)   §7│' },
                    { text: '§7│ §d寰宇并行超限器 → 超限模式(MAX并行)      §7│' },
                    { text: '§7└─────────────────────────────┘' }
            ]);
            } else {
                text.add('§7§o按住 SHIFT 查看详细并行表');
            }
        });
    });
    ...
    // I级 原初物质重组核心
    ItemEvents.tooltip(function(e) {
        e.addAdvanced('gt_shanhai:primordial_matter_recombinator_core', function(item, _, text) {
            if (e.shift) {
                addLore(text, [
                { text: '§7┌─ 提升物品 ─────────────────────────────┐' },
                    { text: '§7│ §f无物品              → §7并行: §f64          §7│' },
    ...
    // II级 原初因果编织矩阵
    ItemEvents.tooltip(function(e) {
        e.addAdvanced('gt_shanhai:primordial_causal_weaving_matrix', function(item, _, text) {
            if (e.shift) {
                addLore(text, [
                { text: '§7┌─ 提升物品 ─────────────────────────────┐' },
                    { text: '§7│ §f无物品              → §7并行: §f64          §7│' },
    ...
    // III级 原初奇点反演核心
    ItemEvents.tooltip(function(e) {
        e.addAdvanced('gt_shanhai:primordial_singularity_inversion_core', function(item, _, text) {
            if (e.shift) {
                addLore(text, [
                { text: '§7┌─ 提升物品 ─────────────────────────────┐' },
                    { text: '§7│ §f无物品              → §7并行: §f64          §7│' },
    ...
    // 原初世界碎片采集器
    ItemEvents.tooltip(function(e) {
        e.addAdvanced('gt_shanhai:primordial_world_fragments_collector', function(item, _, text) {
            if (e.shift) {
                addLore(text, [
                    { text: '§7┌─ 提升物品 ─────────────────────────────┐' },
                    { text: '§7│ §f无物品              → §7并行: §f64          §7│' },
...
ItemEvents.tooltip(function(e) {
    e.addAdvanced('gt_shanhai:taixu_smelting_furnace', function(item, _, text) {
        if (e.shift) {
            addLore(text, [
                { text: '§7┌─ 提升物品 ─────────────────────────────┐' },
                    { text: '§7│ §f无物品              → §7并行: §f64          §7│' },
...
ItemEvents.tooltip(function(e) {
    e.addAdvanced('gt_shanhai:primordial_anti_entropy_condensation_core', function(item, _, text) {
        if (e.shift) {
            addLore(text, [
                { text: '§7┌─ 提升物品 ─────────────────────────────┐' },
                    { text: '§7│ §f无物品              → §7并行: §f64          §7│' },
...
ItemEvents.tooltip(function(e) {
    e.addAdvanced("gt_shanhai:primordial_matter_caster", function(item, _, text) {
        if (e.shift) {
            addLore(text, [
                { text: '§7┌─ 提升物品 ─────────────────────────────┐' },
                    { text: '§7│ §f无物品              → §7并行: §f64          §7│' },
...
ItemEvents.tooltip(function(e) {
    e.addAdvanced("gt_shanhai:primordial_divergence_generator", function(item, _, text) {
        if (e.shift) {
            addLore(text, [
                { text: '§7┌─ 提升物品 ─────────────────────────────┐' },
                    { text: '§7│ §f无物品              → §7并行: §f64          §7│' },
...
ItemEvents.tooltip(function(e) {
    e.addAdvanced("gt_shanhai:primordial_chaotic_ephemeral_deconstruction_crystallization_furnace", function(item, _, text) {
        if (e.shift) {
            addLore(text, [
                { text: '§7┌─ 提升物品 ─────────────────────────────┐' },
                    { text: '§7│ §f无物品              → §7并行: §f64          §7│' },
...
ItemEvents.tooltip(function(e) {
    e.addAdvanced("gt_shanhai:primordial_biological_core", function(item, _, text) {
        if (e.shift) {
            addLore(text, [
                { text: '§7┌─ 提升物品 ─────────────────────────────┐' },
                    { text: '§7│ §f无物品              → §7并行: §f64          §7│' },
...
ItemEvents.tooltip(function(e) {
    e.addAdvanced("gtlcore:pattern_modifier", function(item, _, text) {
        if (e.shift) {
            addLore(text, [
                { inlineText: 'Ω-特殊修改：{electric}【为供应器样板修改器添加单独的配方输出倍率功能】{/}'},
                { inlineText: '│输出额外倍率按键输入对应倍率，输出额外除数用于调整输出除 '},
                { inlineText: '│公式：最终产出 = 原始产出 × Scale ÷ DivScale × 输出× ÷ 输出÷ '},

            ]);
        } else {
            addLore(text, [
                { inlineText: 'Ω| 按住 SHIFT 查看神私修改提示{/}'},
        ])}
    });
})

ItemEvents.tooltip(function(e) {
    e.addAdvanced("gt_shanhai:super_parallel_core", function(item, _, text) {
        if (e.shift) {
            addLore(text, [
                { inlineText: 'Ω-特殊修改：{electric}【添加分操超限模式(long并行)】{/}'},
                { inlineText: '│安装至分子操纵者核心位置[分操内部]后解锁分操超限模式(long并行)'},
...
ItemEvents.tooltip(function(e) {
    e.addAdvanced("gtceu:molecular_assembler_matrix", function(item, _, text) {
        if (e.shift) {
            addLore(text, [
                { inlineText: 'Ω-特殊修改：{electric}【添加分操超限模式(long并行)，需要超级并行核心】{/}'},
                { inlineText: '│安装至分子操纵者核心位置[分操内部]后解锁分操超限模式'},
...
ItemEvents.tooltip(function(e) {
    e.addAdvanced("gtladditions:dimension_focus_infinity_crafting_array", function(item, _, text) {
        if (e.shift) {
            addLore(text, [
                { text: ''},
                { inlineText: 'ω-BUG提示：{electric}【维度聚焦合成阵列有掉并行问题】{/}'},
                { inlineText: '│此问题在add265fix1修复，不过你也可以选择超级并行核心来解决'},

            ]);
        } else {
            addLore(text, [
                { text: ''},
                { inlineText: 'ω| 按住 SHIFT 查看神私BUG提示{/}'},
        ])}
    });
})
...
ItemEvents.tooltip(function(e) {
    e.addAdvanced("gtladditions:dimension_focus_infinity_crafting_array", function(item, _, text) {
        if (e.shift) {
            addLore(text, [
                { text: ''},
                { inlineText: 'Ω-普通提示：{electric}【超级样板总成虽然自带隔离 但其并非绝对不会串配方】{/}'},
                { inlineText: '│关于“隔离”,即将各仓室内的物品分开管理 或说使另一个仓室无法访问超级样板总成 反之亦然'},
                { inlineText: '│其隔离是对外部仓室而言 对其内部的样板并不包含此效果'},
                { inlineText: '│如果你串了配方 把样板放到另一个样板总成 是更好选择'},

            ]);
        } else {
            addLore(text, [
                { text: ''},
                { inlineText: 'Ω| 按住 SHIFT 查看神私普通提示{/}'},
        ])}
    });
})
