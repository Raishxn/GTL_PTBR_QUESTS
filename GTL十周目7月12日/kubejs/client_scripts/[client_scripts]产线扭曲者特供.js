(function() {
    ...
    JEIEvents.addItems(event => {
        ...
        if (config.enableInfinityCellRecipes) {
            ...
            infinityCount++;
            log('Adicionado componente infinito Stargate Crystal Slurry');
            log(`已添加 ${infinityCount} 个无限元件物品到 JEI`);
        }
        if (config.enableGTLAdditionsUpgrade) {
            event.add(Item.of('gtladditions:forge_of_the_antichrist', `{BlockEntityTag:{runningSecs:144000L},display:{Name:'{"text":"A Forja do Falso Deus Final","color":"#FF4500","bold":true,"italic":true}',Lore:['"§6§lForjamento do Falso Deus·Forma Definitiva"','“§e§nApós as intermináveis ​​​​reviravoltas da matriz de flutuação quântica, o poder infinito criou a base do falso deus”','"§7§mTodas as coisas na UE que engoliram 9.2E em um segundo estão chegando ao fim"','“§5§ocarrega todas as coisas e rompe as algemas do mundo”','"§c§kQueime tudo no mundo..."']}}`));
        ...
        if (config.enableComponentPackRecipes) {
            ...
            if (config.enableExclusiveContent) {
                ...
                if (crystalGems.length > 0) {
                    event.add(twisted_packed_infinity_cell('Pacote de componentes de cristal', 'i', crystalGems));
                    log(`已添加晶体元件包，包含 ${crystalGems.length} 种宝石`);
                } else {
                    log('Nenhuma gema fina/perfeita encontrada, pacote de componentes de cristal ignorado');
                }
            }
            
            let nanoswarmItems = Ingredient.of('#forge:nanoswarms').getItemIds();
            if (nanoswarmItems.length > 0) {
                event.add(twisted_packed_infinity_cell('Pacote de componentes Nano Swarm', 'i', nanoswarmItems));
                log(`已添加纳米蜂群元件包，包含 ${nanoswarmItems.length} 种纳米蜂群`);
            } else {
                log('Nenhum nanoswarm encontrado, ignorando o pacote de componentes do nanoswarm');
            }
        }
    });
