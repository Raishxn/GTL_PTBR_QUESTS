...
(function() {
	...
	const machineModifierPrototype = {
		...
		push() {
			...
			// 防蠢
			if (!config.source) {
				console.warn('A fonte [uiEditor Constructor][Serious Error] é necessária, uma determinada adição não está em conformidade com a especificação e toda a configuração é ignorada!')
				return this
			}
			// 拷贝一次, 防蠢
	...
	function createMachineModifier(fieldName) {
		...
		if (!fieldName || typeof fieldName !== 'string') {
			console.warn("[uiEditor Constructor][Serious Error] fieldName é obrigatório, uma determinada adição não está em conformidade com a especificação e toda a configuração é ignorada!")
			return {
				source: function() {
					return this
				},
