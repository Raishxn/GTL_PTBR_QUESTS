...
(function() {
    ...
    if (typeof Java !== 'undefined' && typeof Java.loadClass === 'function') {
        try {
            ...
            console.log('§b[API de controle de receita] Carregamento da classe Java concluído');
        } catch (e) {
            console.log('§e[API de controle de receita] Carregamento de classe Java bloqueado (mecanismo de segurança KubeJS), alternativa JsonIO será usada');
            File = null;
            FileReader = null;
            BufferedReader = null;
        }
    } else {
        console.log('§e[API de controle de receita] API Java não está disponível, JsonIO será usado');
    }
    // 自动配方注册开关
...
var LOG_PREFIX = '§b[API de controle de receita] §r';
...
function createProtectedFunction(originalFunc, functionName, defaultValue) {
    return function() {
        try {
            ...
            // 构建完整的错误报告
            var fullErrorMessage = '❌ Erro global de execução da API - função:' + functionName + '\n' +
                                 'Informação:' + err.message + '\n' +
                                 'Pilha:' + (err.stack || 'Nenhuma informação de pilha') + '\n' +
                                 'parâmetro:' + JSON.stringify(Array.prototype.slice.call(arguments)) + '\n' +
                                 'tempo:' + new Date().toLocaleString();
            
            // 同时发送到多个日志渠道，确保不丢失
            // 1. 发送到控制台（KubeJS会捕获）
            console.error('[Proteção contra erros da API Shanhai]' + fullErrorMessage);
            
            // 2. 发送到自定义日志系统
            error('Captura global de proteção contra erros da API:' + functionName + ' - ' + err.message);
            debug('Detalhes do erro:' + JSON.stringify(errorDetails));
    ...
    for (var key in apiObject) {
        if (apiObject.hasOwnProperty(key)) {
            ...
            if (typeof func === 'function') {
                ...
                debug('Adicionada proteção contra erros às funções:' + key + '(valor padrão:' + JSON.stringify(defaultValue) + ')');
            } else {
                // 非函数属性直接复制
...
// 将Java Map（JsonIO返回）转为原生JS对象，避免Rhino for..in遍历Java索引
    ...
    try {
        ...
        try {
            if (typeof JsonIO !== 'undefined' && typeof JsonIO.read === 'function') {
                defaultConfig = toNativeObject(JsonIO.read(DEFAULT_CONFIG_PATH));
            } else {
                // 使用 Java 文件操作
            ...
            try {
                ...
                if (typeof JsonIO !== 'undefined' && typeof JsonIO.read === 'function') {
                    ...
                    if (fileConfig && typeof fileConfig === 'object') {
                        recipeLoadConfig = toNativeObject(fileConfig);
                        loaded = true;
                                ...
                                if (file.exists()) {
                                    ...
                                    try {
                                        ...
                                        if (parsedConfig && typeof parsedConfig === 'object') {
                                            recipeLoadConfig = toNativeObject(parsedConfig);
                                            loaded = true;
                ...
                try {
                    if (typeof JsonIO !== 'undefined' && typeof JsonIO.read === 'function') {
                        ...
                        if (backupConfig && typeof backupConfig === 'object' && Object.keys(backupConfig).length > 0) {
                            recipeLoadConfig = toNativeObject(backupConfig);
                            backupLoaded = true;
 ...
 * 批量设置配方数组的加载状态
 * 
 * @param {string} arrayName - 配方数组名称（如 'assrecipes', 'universalRecipes' 等）
 * @param {boolean} enabled - 是否启用数组中的所有配方
 * @returns {Object} 结果对象 {success: number, failed: number, errors: Array, arrayName: string, recipeCount: number}
 */
function batchSetArrayEnabled(arrayName, enabled) {
    if (!arrayName || typeof arrayName !== 'string') {
        return { success: 0, failed: 0, errors: ['数组名称必须是字符串'], arrayName: arrayName, recipeCount: 0 };
    }
    
    var recipeArray = global[arrayName];
    if (!recipeArray || !Array.isArray(recipeArray)) {
        return { success: 0, failed: 0, errors: ['找不到配方数组: ' + arrayName + ' 或不是数组'], arrayName: arrayName, recipeCount: 0 };
    }
    ...
    for (var i = 0; i < recipeArray.length; i++) {
        ...
        if (recipe && recipe.id) {
            ...
            configs[recipeId] = enabled;
            recipeCount++;
        }
    }
    
    if (recipeCount === 0) {
        return { success: 0, failed: 0, errors: ['数组中未找到有效配方'], arrayName: arrayName, recipeCount: 0 };
    }
    
    info('开始批量设置数组 "' + arrayName + '" 的加载状态: 共 ' + recipeCount + ' 个配方，目标状态: ' + (enabled ? '启用' : '禁用'));
    
    // 使用现有的批量设置函数
    var result = batchSetRecipeEnabled(configs);
    result.arrayName = arrayName;
    result.recipeCount = recipeCount;
    
    info('数组 "' + arrayName + '" 批量设置完成: 成功 ' + result.success + ' 个，失败 ' + result.failed + ' 个');
    return result;
}
 ...
 * 修改配方
 * 
 * 通过ID查找配方并修改其属性。修改成功后会标记配方层缓存失效，
 * 并自动同步到 GTCEu 注册表，后续使用样板总成缓存/重放样板时会看到新值。
 * 
...
function modifyRecipe(id, modifications) {
    ...
    // 配方层缓存失效：样板总成缓存样板后，下一次配方层重放会重新应用该修改。
    if (typeof global !== 'undefined') {
        global.shanhaiRecipeLayerDirty = true;
        debug('配方层缓存已标记失效，等待样板总成重放时重新应用');
    }

    // 同步到 GTCEu 注册表：移除旧配方并用修改后的数据重建，确保样板总成下次查找时看到新值。
    if (global._shanhaiGTR && global._shanhaiGTR[recipe.type]) {
        try {
            var fullId = recipe.id;
            if (fullId.indexOf(':') === -1) fullId = 'dishanhai:' + fullId;
            var javaAPI = Java.loadClass('com.dishanhai.gt_shanhai.api.DShanhaiRecipeModifierAPI');
            if (typeof javaAPI.removeAndSync === 'function') {
                javaAPI.removeAndSync(recipe.type, fullId);
            }
            var m = global._shanhaiGTR[recipe.type](fullId);
            DShanhaiRecipeEngine.applyAll(m, recipe);
            m.save();
            debug('✅ 配方已同步到 GTCEu 注册表: ' + fullId);
        } catch (syncErr) {
            warn('⚠️ 配方同步到 GTCEu 注册表失败: ' + syncErr.message);
        }
    }
...
function handleRecipeSwitchCommand(player, args) {
    debug('handleRecipeSwitchCommand é chamado, argumentos:' + JSON.stringify(args));
    
    if (!isPlayerOp(player)) {
        sendMessageToPlayer(player, 'Erro §c: Somente jogadores OP podem usar este comando!');
        return false;
    }
    
    if (!args || args.length < 2) {
        sendMessageToPlayer(player, 'Uso de §e: /recipe switch <recipe ID> <on/off>');
        sendMessageToPlayer(player, 'Exemplo §7: /recipe switch test_recipe_load_control desativado');
        sendMessageToPlayer(player, '§7Valores disponíveis: ativado, ativado, ativado, verdadeiro, 1, desativado, desativado, desativado, falso, 0');
        return false;
    }
    ...
    debug('recipeId: ' + recipeId + '(tipo:' + typeof args[0] + ' -> ' + typeof recipeId + '), switchValue: ' + switchValue);
    
    if (!recipeId || recipeId.trim() === '') {
        sendMessageToPlayer(player, 'Erro §c: O ID da receita não pode estar vazio!');
        return false;
    }
    
    // 解析开关值
    var enabled;
    if (['abrir', 'ligar', 'habilitar', 'true', '1', 'on', 'enable'].includes(switchValue)) {
        enabled = true;
    } else if (['fechar', 'encerramento', 'Desativar', 'false', '0', 'off', 'disable'].includes(switchValue)) {
        enabled = false;
    } else {
        sendMessageToPlayer(player, 'Erro §c: valor de chave inválido, use "on" ou "off"');
        sendMessageToPlayer(player, '§7Valores disponíveis: ativado, ativado, ativado, verdadeiro, 1, desativado, desativado, desativado, falso, 0');
        return false;
    }
    
    sendMessageToPlayer(player, '§aestá configurando o status de carregamento da receita: §e' + recipeId);
    sendMessageToPlayer(player, 'Status §7: §e' + (enabled ? 'ligar (habilitar)' : 'Desligado (desativado)'));
    
    debug('Chame setRecipeEnabled("' + recipeId + '", ' + enabled + ')');
    var result = setRecipeEnabled(recipeId, enabled);
    debug('setRecipeEnabled retorna:' + result);
    
    if (result) {
        sendMessageToPlayer(player, '§a✅ Status de carregamento da receita definido com sucesso!');
        sendMessageToPlayer(player, '§7Nota: Esta configuração controla se a receita deve ser carregada; o efeito real depende da implementação do script.');
    } else {
        sendMessageToPlayer(player, '§c❌ Falha na configuração do status de carregamento da receita!');
        sendMessageToPlayer(player, '§7Possíveis motivos: O ID da receita é inválido ou ocorreu um erro ao salvar a configuração.');
        sendMessageToPlayer(player, '§7Use /recipe diagnostics para verificar o status da API');
    }
 ...
 * 
 * 格式: /配方数组开关 <数组名> <开/关>
 * 示例: /配方数组开关 assrecipes 关
 * 
 * @param {Object} player - 玩家对象
 * @param {Array} args - 命令参数
 * @returns {boolean} 是否处理成功
 */
function handleArraySwitchCommand(player, args) {
    debug('handleArraySwitchCommand é chamado, argumentos:' + JSON.stringify(args));
    ...
    if (!args || args.length < 2) {
        sendMessageToPlayer(player, '§eUso: /Recipe array switch <nome do array> <ligado/desligado>');
        sendMessageToPlayer(player, 'Exemplo §7: matriz /recipe desativa assrecipes');
        sendMessageToPlayer(player, '§7Nomes de array disponíveis: assrecipes, universalRecipes, suprecipes_1, Dishanhairecipes, Recipes, Recipes_electrolyzers');
        sendMessageToPlayer(player, '§7Valores disponíveis: ativado, ativado, ativado, verdadeiro, 1, desativado, desativado, desativado, falso, 0');
        return false;
    }
    
    // 确保参数是字符串（支持Java字符串对象）
    var arrayName = String(args[0]);
    var switchValue = String(args[1]).toLowerCase();
    
    debug('arrayName: ' + arrayName + ', switchValue: ' + switchValue);
    
    if (!arrayName || arrayName.trim() === '') {
        sendMessageToPlayer(player, 'Erro §c: o nome da matriz não pode estar vazio!');
        return false;
    }
    ...
    sendMessageToPlayer(player, '§aestá configurando o status de carregamento do array: §e' + arrayName);
    sendMessageToPlayer(player, 'Status §7: §e' + (enabled ? 'ligar (habilitar)' : 'Desligado (desativado)'));
    sendMessageToPlayer(player, '§7Esta operação definirá em lote o status de carregamento de todas as receitas na matriz...');
    
    debug('Chame batchSetArrayEnabled("' + arrayName + '", ' + enabled + ')');
    var result = batchSetArrayEnabled(arrayName, enabled);
    debug('batchSetArrayEnabled retorna:' + JSON.stringify(result));
    
    sendMessageToPlayer(player, '§6═══════════════════════════════════════════════════════════');
    sendMessageToPlayer(player, '§aConfiguração de status de carregamento do array concluída: §e' + arrayName);
    sendMessageToPlayer(player, 'Existem receitas na matriz §7: §e' + result.recipeCount + 'individual');
    sendMessageToPlayer(player, '§7definido com sucesso: §a' + result.success + 'individual');
    
    if (result.failed > 0) {
        sendMessageToPlayer(player, 'Falha na configuração de §c: §e' + result.failed + 'individual');
        if (result.errors && result.errors.length > 0) {
            sendMessageToPlayer(player, '§7Primeiros 3 erros:');
            for (var i = 0; i < Math.min(3, result.errors.length); i++) {
                sendMessageToPlayer(player, '§c  • ' + result.errors[i]);
            }
        }
    }
    
    sendMessageToPlayer(player, '§7Nota: Esta configuração controla se todas as receitas no array devem ser carregadas; o efeito real depende da implementação do script.');
    sendMessageToPlayer(player, '§7Você ainda pode definir o status de carregamento de cada receita individualmente, substituindo esta configuração de lote.');
    
    if (result.success > 0) {
        sendMessageToPlayer(player, '§a✅ Configuração de lote de status de carregamento da matriz bem-sucedida!');
        return true;
    } else {
        sendMessageToPlayer(player, '§c❌ Falha na configuração do status de carregamento do array!');
        sendMessageToPlayer(player, '§7Possíveis razões: O array não existe, o array está vazio ou erro interno.');
        sendMessageToPlayer(player, '§7Use /recipe diagnostics para verificar o status da API');
        return false;
    }
}
...
function handleRecipeStatusCommand(player, args) {
    ...
    if (args.length < 1) {
        sendMessageToPlayer(player, 'Uso de §e: /status da receita <ID da receita>');
        sendMessageToPlayer(player, 'Exemplo §7: /status da receita minha_receita');
        return false;
    }
    
    var recipeId = args[0];
    
    sendMessageToPlayer(player, '§aConsultando o status de carregamento da receita: §e' + recipeId);
    ...
    sendMessageToPlayer(player, '§6═══════════════════════════════════════════════════════════');
    sendMessageToPlayer(player, '§aStatus de carregamento da receita: §e' + recipeId);
    sendMessageToPlayer(player, '§7Status atual: §e' + (enabled ? 'ligar (habilitar)' : 'Desligado (desativado)'));
    sendMessageToPlayer(player, '§7Definições de configuração: §e' + (hasConfig ? 'configurado' : 'Não configurado (use valor padrão)'));
    
    if (hasConfig) {
        var configValue = recipeLoadConfig[configKey];
        sendMessageToPlayer(player, 'Chave de configuração §7: §e' + configKey);
        sendMessageToPlayer(player, 'Valor de configuração §7: §e' + (configValue ? 'true' : 'false'));
    } else {
        sendMessageToPlayer(player, '§7Valor padrão: §etrue (todas as receitas estão habilitadas por padrão)');
    }
    
    sendMessageToPlayer(player, '§6═══════════════════════════════════════════════════════════');
    sendMessageToPlayer(player, '§7usa chave §e/receita' + recipeId + ' ' + (enabled ? 'fechar' : 'abrir') + 'Status do interruptor §7');
...
function handleEnabledListCommand(player) {
    ...
    sendMessageToPlayer(player, '§6═══════════════════════════════════════════════════════════');
    sendMessageToPlayer(player, '§aLista de receitas habilitadas');
    sendMessageToPlayer(player, '§6───────────────────────────────────────────────────────────');
    
    if (enabledRecipes.length === 0) {
        sendMessageToPlayer(player, '§7Nenhuma receita definida explicitamente como habilitada');
        sendMessageToPlayer(player, '§7NOTA: Todas as receitas são habilitadas por padrão, a menos que explicitamente definidas como desabilitadas');
    } else {
        sendMessageToPlayer(player, 'Quantidade §7: §e' + enabledRecipes.length + 'receitas');
        sendMessageToPlayer(player, '§6───────────────────────────────────────────────────────────');
    ...
    sendMessageToPlayer(player, '§6───────────────────────────────────────────────────────────');
    sendMessageToPlayer(player, '§7Use §e/recipe status <recipe ID> §7para visualizar o status detalhado');
    sendMessageToPlayer(player, '§7Usar §e/lista de receitas desabilitada §7Ver lista de receitas desabilitadas');
    sendMessageToPlayer(player, '§6═══════════════════════════════════════════════════════════');
...
function handleDisabledListCommand(player) {
    ...
    sendMessageToPlayer(player, '§6═══════════════════════════════════════════════════════════');
    sendMessageToPlayer(player, 'Lista de receitas desabilitadas §a');
    sendMessageToPlayer(player, '§6───────────────────────────────────────────────────────────');
    
    if (disabledRecipes.length === 0) {
        sendMessageToPlayer(player, '§7Nenhuma receita definida explicitamente como desativada');
        sendMessageToPlayer(player, '§7Nota: Todas as receitas estão habilitadas por padrão');
    } else {
        sendMessageToPlayer(player, 'Quantidade §7: §e' + disabledRecipes.length + 'receitas');
        sendMessageToPlayer(player, '§6───────────────────────────────────────────────────────────');
    ...
    sendMessageToPlayer(player, '§6───────────────────────────────────────────────────────────');
    sendMessageToPlayer(player, '§7Use §e/recipe status <recipe ID> §7para visualizar o status detalhado');
    sendMessageToPlayer(player, '§7Use §e/recipe switch <recipe ID> para abrir §7habilitar receita');
    sendMessageToPlayer(player, '§6═══════════════════════════════════════════════════════════');
...
function handleRecipeConfigResetCommand(player) {
    ...
    if (configCount === 0) {
        sendMessageToPlayer(player, 'Dica §e: A configuração de carregamento da receita já está vazia e não precisa ser redefinida.');
        return true;
    }
    
    sendMessageToPlayer(player, '§c⚠️ Aviso: a configuração de carregamento de receitas está prestes a ser redefinida!');
    sendMessageToPlayer(player, '§7Isso limpará tudo' + configCount + 'Carregar configurações para receitas');
    sendMessageToPlayer(player, '§7Todas as receitas serão restauradas ao seu estado habilitado padrão');
    sendMessageToPlayer(player, '§cEsta operação é irreversível!');
    sendMessageToPlayer(player, '§7Insira §e/Redefinição de confirmação de receita §7Continuar operação');
    
    // 设置确认标记
    global.shanhaiRecipeConfigResetPending = true;
    global.shanhaiRecipeConfigResetPlayer = player.username || player.name || 'jogador desconhecido';
...
function handleRecipeConfirmResetCommand(player) {
    ...
    if (!global.shanhaiRecipeConfigResetPending) {
        sendMessageToPlayer(player, 'Dica §e: Nenhuma operação de redefinição de configuração pendente');
        sendMessageToPlayer(player, '§7Use o comando §e/recipe reset configuration §7primeiro');
        return false;
    }
    
    var pendingPlayer = global.shanhaiRecipeConfigResetPlayer;
    var currentPlayer = player.username || player.name || 'jogador desconhecido';
    
    if (pendingPlayer !== currentPlayer) {
        sendMessageToPlayer(player, 'Erro §c: Esta operação de reinicialização foi realizada pelo jogador §e' + pendingPlayer + 'Iniciado por §c');
        sendMessageToPlayer(player, '§cSomente o iniciador pode confirmar a operação de reinicialização');
        return false;
    }
    
    var configCount = Object.keys(recipeLoadConfig).length;
    
    sendMessageToPlayer(player, '§aRedefinindo configuração de carregamento de receita...');
    ...
    if (saved) {
        sendMessageToPlayer(player, '§a✅ Todas as configurações da receita foram apagadas!');
        sendMessageToPlayer(player, '§7apagado' + configCount + 'Carregar configurações para receitas');
        sendMessageToPlayer(player, '§7Todas as receitas voltarão ao comportamento padrão');
    } else {
        sendMessageToPlayer(player, '§c❌ Falha ao limpar a configuração de carregamento da receita!');
        sendMessageToPlayer(player, '§7Possíveis motivos: Erro ao salvar a configuração');
    }
...
function handleRecipeDiagnosticCommand(player) {
    ...
    sendMessageToPlayer(player, '§6=== Diagnóstico da API de controle de receita ===');
    
    // 显示配置基本信息
    sendMessageToPlayer(player, 'Entradas de RecipeLoadConfig §7: §e' + Object.keys(recipeLoadConfig).length);
    
    var keys = Object.keys(recipeLoadConfig);
    if (keys.length > 0) {
        sendMessageToPlayer(player, 'Receitas §7para as primeiras 10 configurações:');
        for (var i = 0; i < Math.min(keys.length, 10); i++) {
            var key = keys[i];
            var value = recipeLoadConfig[key];
            sendMessageToPlayer(player, '§7  ' + key + ' = ' + (value ? '§aativado' : '§cdesativado'));
        }
    }
    
    // 检查全局存储
    sendMessageToPlayer(player, 'Verificação §7global.shanhaiRecipeLoadConfig:' + 
        (typeof global !== 'undefined' ? 
            (global.shanhaiRecipeLoadConfig ? '§aé definido' : '§cindefinido') : 
            'O objeto global §cé indefinido'));
    
    if (typeof global !== 'undefined' && global.shanhaiRecipeLoadConfig) {
        var globalKeys = Object.keys(global.shanhaiRecipeLoadConfig);
        sendMessageToPlayer(player, '§7Número de entradas de armazenamento global: §e' + globalKeys.length);
    }
    
    // 检查 JsonIO API
    sendMessageToPlayer(player, 'Verificação da API §7JsonIO:' + 
        (typeof JsonIO !== 'undefined' ? '§adisponível' : '§cnão está disponível'));
    
    // 检查配方数组导出
    sendMessageToPlayer(player, 'Verificação de exportação da matriz de receita §7:' + 
        (typeof global !== 'undefined' && global.shanhaiRecipeArrays ? 
            '§aexportado (' + Object.keys(global.shanhaiRecipeArrays).length + 'variedade)' : 
            '§cnão exportado'));
    
    // 检查自动注册设置
    var autoRegisterEnabled = typeof global !== 'undefined' && global.AUTO_REGISTER_RECIPES !== false;
    sendMessageToPlayer(player, 'Registro automático de receita §7:' + (autoRegisterEnabled ? '§aativado' : '§cdesativado'));
    
    // 测试保存功能
    sendMessageToPlayer(player, '§7está testando a função salvar...');
    var saveResult = saveRecipeLoadConfig();
    sendMessageToPlayer(player, '§7Salvar resultados do teste:' + (saveResult ? 'Sucesso §a' : '§cfalhou'));
    
    // 如果配置为空，提供建议
    if (Object.keys(recipeLoadConfig).length === 0) {
        sendMessageToPlayer(player, '§6=== Configuração com prompt vazio ===');
        sendMessageToPlayer(player, '§7atualmente não possui nenhuma receita configurada, o que pode ser devido aos seguintes motivos:');
        sendMessageToPlayer(player, '§71. Primeira execução, nenhuma receita foi configurada ainda');
        sendMessageToPlayer(player, '§72. A função de registro automático está desativada');
        sendMessageToPlayer(player, '§73. A matriz de receitas não foi exportada para o objeto global');
        sendMessageToPlayer(player, 'Ação recomendada §7:');
        sendMessageToPlayer(player, '§7- Use §e/lista de receitas §7para ver todas as receitas disponíveis');
        sendMessageToPlayer(player, '§7- Use §e/recipe switch <recipe ID> para ligar/desligar §7para controlar o carregamento da receita');
        if (global.shanhaiRecipeControlAPI) {
            sendMessageToPlayer(player, '§7- Use §eglobal.shanhaiRecipeControlAPI.autoRegisterRecipesFromArrays() §7para registrar manualmente');
        }
    }
    
    // 诊断结果
    sendMessageToPlayer(player, '§6=== Diagnóstico concluído ===');
    sendMessageToPlayer(player, '§7Se o salvamento falhar, verifique as permissões do arquivo e o espaço em disco');
...
function handleRecipeScanRegisterCommand(player) {
    ...
    sendMessageToPlayer(player, '§6=== Registro de digitalização de receita ===');
    sendMessageToPlayer(player, '§7está escaneando a matriz de receitas e registrando automaticamente a receita...');
    
    // 检查配方数组是否已导出
    if (typeof global === 'undefined' || !global.shanhaiRecipeArrays) {
        sendMessageToPlayer(player, 'Erro §c: A matriz de receitas não foi exportada para o objeto global!');
        sendMessageToPlayer(player, '§7Isso pode ocorrer porque o script da receita não foi totalmente carregado.');
        sendMessageToPlayer(player, '§7Recomenda-se aguardar a inicialização completa do servidor antes de executar este comando.');
        return false;
    }
    
    // 获取当前配置状态
    var beforeCount = Object.keys(recipeLoadConfig).length;
    sendMessageToPlayer(player, '§7Número de entradas de configuração atuais: §e' + beforeCount);
    sendMessageToPlayer(player, '§7Número de matrizes de receita: §e' + Object.keys(global.shanhaiRecipeArrays).length);
    ...
    try {
        ...
        if (registeredCount > 0) {
            sendMessageToPlayer(player, '§a✅ Registro de digitalização concluído!');
            sendMessageToPlayer(player, '§7Novo registro: §e' + registeredCount + 'Receitas §7');
            
            // 获取更新后的配置状态
            var afterCount = Object.keys(recipeLoadConfig).length;
            sendMessageToPlayer(player, '§7Alteração no número de entradas de configuração: §e' + beforeCount + ' §7→ §a' + afterCount);
            
            // 尝试保存配置
            sendMessageToPlayer(player, '§7Salvando configuração...');
            var saved = saveRecipeLoadConfig();
            if (saved) {
                sendMessageToPlayer(player, '§a✅ Configuração salva em arquivo com sucesso');
            } else {
                sendMessageToPlayer(player, '§c⚠️ Falha ao salvar a configuração, mas a configuração na memória foi atualizada');
            }
        } else {
            sendMessageToPlayer(player, '§7Varredura concluída, não foram encontradas novas fórmulas que precisam ser registradas.');
            sendMessageToPlayer(player, '§7A configuração atual contém todas as receitas na matriz de receitas');
            
            // 检查是否有跳过或保留的配方
            // 这里可以添加更详细的统计信息
        }
        
        sendMessageToPlayer(player, '§6=== Registro de digitalização concluído ===');
        return true;
    } catch (err) {
        sendMessageToPlayer(player, '§c❌ Ocorreu um erro durante o processo de registro da digitalização!');
        sendMessageToPlayer(player, 'Mensagem de erro §7: §c' + err.message);
        error('Falha no registro da verificação de receita:' + err.message);
        return false;
    }
}
...
ServerEvents.commandRegistry(function(event) {
    ...
    // 注册 /配方修改 命令
    event.register( 
        Commands.literal('Modificação de receita')
            ...
            .then(Commands.argument('args', Arguments.GREEDY_STRING.create(event))
                .executes(function(ctx) {
                    ...
                    if (player) {
                        ...
                        // 控制台执行
                        console.log('Modificação da receita de execução do console:' + argsStr);
                        if (args.length >= 3) {
                            var result = updateRecipeField(args[0], args[1], args[2]);
                            console.log('resultado:' + (result ? 'sucesso' : 'falhar'));
                        }
                    }
                    return 1;
                })
    ...
    // 注册 /配方数组开关 命令
    event.register(
        Commands.literal('Troca de matriz de receita')
            .requires(function(source) {
                if (source.getEntity && source.getEntity()) {
                    var player = source.getEntity();
                    return isPlayerOp(player);
            ...
            .then(Commands.argument('args', Arguments.GREEDY_STRING.create(event))
                .executes(function(ctx) {
                    ...
                    if (player) {
                        handleArraySwitchCommand(player, args);
                    } else {
                        // 控制台执行
                        console.log('Mudança de matriz de receita de execução do console:' + argsStr);
                        if (args.length >= 2) {
                            var arrayName = args[0];
                            var switchValue = args[1].toLowerCase();
                            var enabled = ['abrir', 'ligar', 'habilitar', 'true', '1', 'on', 'enable'].includes(switchValue);
                            var result = batchSetArrayEnabled(arrayName, enabled);
                            console.log('Resultado: Sucesso' + result.success + 'um, falhou' + result.failed + 'individual');
                        }
                    }
                    return 1;
                })
    ...
    // 注册 /配方信息 命令
    event.register(
        Commands.literal('Informações da receita')
            .requires(function(source) {
                if (source.getEntity && source.getEntity()) {
                    var player = source.getEntity();
                    return isPlayerOp(player);
            ...
            .then(Commands.argument('id', Arguments.GREEDY_STRING.create(event))
                .executes(function(ctx) {
                    ...
                    if (player) {
                        ...
                        var info = getRecipeInfo(recipeId);
                        console.log('Informações da receita:' + JSON.stringify(info, null, 2));
                    }
                    return 1;
                })
            )
    ...
    // 注册 /配方列表 命令
    event.register(
        Commands.literal('Lista de receitas')
            .requires(function(source) {
                if (source.getEntity && source.getEntity()) {
                    var player = source.getEntity();
                    return isPlayerOp(player);
            ...
            .executes(function(ctx) {
                ...
                if (player) {
                    ...
                    console.log('Lista de matrizes de receitas:');
                    ...
                    for (var i = 0; i < recipeArrays.length; i++) {
                        ...
                        if (arr) console.log(recipeArrays[i] + ': ' + arr.length + 'receitas');
                    }
                }
                return 1;
            })
    ...
    // 注册 /配方帮助 命令
    event.register(
        Commands.literal('Ajuda de receita')
            .requires(function(source) {
                if (source.getEntity && source.getEntity()) {
                    var player = source.getEntity();
                    return isPlayerOp(player);
            ...
            .executes(function(ctx) {
                ...
                if (player) {
                    ...
                    console.log('Ajuda da API de controle de receitas - Comandos disponíveis: /Modificação de receitas, /Informações de receitas, /Lista de receitas, /Ajuda de receitas');
                }
                return 1;
            })
    );
    ...
    // 注册 /配方开关 命令
    event.register(
        Commands.literal('Troca de receita')
            .requires(function(source) {
                if (source.getEntity && source.getEntity()) {
                    var player = source.getEntity();
                    return isPlayerOp(player);
                ...
                .executes(function(ctx) {
                    ...
                    if (args && typeof args === 'object' && !Array.isArray(args)) {
                        try {
                            ...
                            args = jsArgs;
                            debug('Matriz Java convertida em matriz JavaScript, comprimento:' + args.length);
                        } catch (err) {
                            debug('Falha na conversão da matriz Java:' + err.message);
                        }
                    }
                    
                    debug('Execução do comando de troca de receita, argsStr: "' + argsStr + '", args: ' + JSON.stringify(args));
                    
                    if (player) {
                        handleRecipeSwitchCommand(player, args);
                    } else {
                        console.log('Mudança de receita de execução do console:' + argsStr);
                        if (args.length >= 2) {
                            var enabled;
                            var switchValue = String(args[1]).toLowerCase();
                            if (['abrir', 'ligar', 'habilitar', 'true', '1', 'on', 'enable'].includes(switchValue)) {
                                ...
                                console.log('Erro: valor de opção inválido');
                                return 1;
                            }
    ...
    // 注册 /配方状态 命令
    event.register(
        Commands.literal('Estado da receita')
            .requires(function(source) {
                if (source.getEntity && source.getEntity()) {
                    var player = source.getEntity();
                    return isPlayerOp(player);
            ...
            .then(Commands.argument('id', Arguments.GREEDY_STRING.create(event))
                .executes(function(ctx) {
                    ...
                    if (player) {
                        ...
                        var enabled = isRecipeEnabled(recipeId);
                        console.log('Estado da receita:' + recipeId + ' = ' + (enabled ? 'habilitar' : 'Desativar'));
                    }
                    return 1;
                })
            )
    ...
    // 注册 /配方列表已启用 命令
    event.register(
        Commands.literal('A lista de receitas está ativada')
            .requires(function(source) {
                if (source.getEntity && source.getEntity()) {
                    var player = source.getEntity();
                    return isPlayerOp(player);
            ...
            .executes(function(ctx) {
                ...
                if (player) {
                    ...
                    var enabledRecipes = getEnabledRecipes();
                    console.log('Lista de receitas habilitada:' + enabledRecipes.length + 'individual');
    ...
    // 注册 /配方列表已禁用 命令
    event.register(
        Commands.literal('Lista de receitas desativada')
            .requires(function(source) {
                if (source.getEntity && source.getEntity()) {
                    var player = source.getEntity();
                    return isPlayerOp(player);
            ...
            .executes(function(ctx) {
                ...
                if (player) {
                    ...
                    var disabledRecipes = getDisabledRecipes();
                    console.log('Lista de receitas desabilitadas:' + disabledRecipes.length + 'individual');
    ...
    // 注册 /配方重置配置 命令
    event.register(
        Commands.literal('Configuração de redefinição de receita')
            .requires(function(source) {
                if (source.getEntity && source.getEntity()) {
                    var player = source.getEntity();
                    return isPlayerOp(player);
            ...
            .executes(function(ctx) {
                ...
                if (player) {
                    ...
                    console.log('Console executa redefinição de configuração de receita');
                    var configCount = Object.keys(recipeLoadConfig).length;
                    if (configCount > 0) {
                        console.log('Aviso: será apagado' + configCount + 'Carregar configurações para receitas');
                        console.log('Digite o comando de confirmação para continuar...');
                    } else {
                        console.log('Dica: A configuração de carregamento da receita já está vazia');
                    }
                }
                return 1;
            })
    ...
    // 注册 /配方确认重置 命令
    event.register(
        Commands.literal('Redefinição de confirmação de receita')
            .requires(function(source) {
                if (source.getEntity && source.getEntity()) {
                    var player = source.getEntity();
                    return isPlayerOp(player);
            ...
            .executes(function(ctx) {
                ...
                if (player) {
                    ...
                    console.log('Redefinição de confirmação da receita de execução do console');
                    if (global.shanhaiRecipeConfigResetPending) {
                        var result = resetRecipeLoadConfig();
                        global.shanhaiRecipeConfigResetPending = false;
                        console.log('resultado:' + (result ? 'sucesso' : 'falhar'));
                    } else {
                        console.log('Erro: Nenhuma operação de redefinição de configuração pendente');
                    }
                }
                return 1;
            })
    ...
    // 注册 /配方诊断 命令
    event.register(
        Commands.literal('Diagnóstico de receita')
            .requires(function(source) {
                if (source.getEntity && source.getEntity()) {
                    var player = source.getEntity();
                    return isPlayerOp(player);
            ...
            .executes(function(ctx) {
                ...
                if (player) {
                    ...
                    console.log('Diagnóstico de receita de execução do console');
                    console.log('Número de entradas de RecipeLoadConfig:' + Object.keys(recipeLoadConfig).length);
                    console.log('Verificação global.shanhaiRecipeLoadConfig:' + 
                        (typeof global !== 'undefined' ? 
                            (global.shanhaiRecipeLoadConfig ? 'definido' : 'indefinido') : 
                            'objeto global é indefinido'));
                    console.log('Verificação da API JsonIO:' + (typeof JsonIO !== 'undefined' ? 'Disponível' : 'Não disponível'));
                }
                return 1;
            })
    );
    ...
    // 注册 /配方扫描注册 命令
    event.register(
        Commands.literal('Registro de digitalização de receitas')
            .requires(function(source) {
                if (source.getEntity && source.getEntity()) {
                    var player = source.getEntity();
                    return isPlayerOp(player);
            ...
            .executes(function(ctx) {
                ...
                if (player) {
                    ...
                    console.log('Execução do console do registro de varredura de receita');
                    console.log('Digitalizando matriz de receitas e registrando receitas automaticamente...');
                    
                    // 检查配方数组是否已导出
                    if (typeof global === 'undefined' || !global.shanhaiRecipeArrays) {
                        console.log('Erro: O array de receitas não foi exportado para o objeto global!');
                        console.log('Isso pode ocorrer porque o script da receita não foi totalmente carregado.');
                        return 1;
                    }
                    
                    // 获取当前配置状态
                    var beforeCount = Object.keys(recipeLoadConfig).length;
                    console.log('Número atual de entradas de configuração:' + beforeCount);
                    console.log('Número de matrizes de receitas:' + Object.keys(global.shanhaiRecipeArrays).length);
                    ...
                    try {
                        ...
                        if (registeredCount > 0) {
                            console.log('✅Registro de digitalização concluído!');
                            console.log('Novo registro:' + registeredCount + 'receitas');
                            
                            // 获取更新后的配置状态
                            var afterCount = Object.keys(recipeLoadConfig).length;
                            console.log('Mudanças no número de entradas de configuração:' + beforeCount + ' → ' + afterCount);
                            
                            // 尝试保存配置
                            console.log('Salvando configuração...');
                            var saved = saveRecipeLoadConfig();
                            if (saved) {
                                console.log('✅ Configuração salva com sucesso em arquivo');
                            } else {
                                console.log('⚠️ Falha ao salvar a configuração, mas a configuração na memória foi atualizada');
                            }
                        } else {
                            console.log('Digitalização concluída, não foram encontradas novas receitas que precisam ser registradas.');
                            console.log('A configuração atual já contém todas as receitas na matriz de receitas');
                        }
                        
                        console.log('=== Registro de digitalização concluído ===');
                    } catch (err) {
                        console.log('❌ Ocorreu um erro durante o processo de registro da digitalização!');
    ...
    info('O comando de bate-papo da API de controle de receita foi registrado (incluindo o comando de controle de carregamento de receita)');
});
...
// 使用错误保护包装器保护所有API函数
info('Adicionando proteção contra erros à API global...');
global.shanhaiRecipeControlAPI = protectAllAPIFunctions(rawAPI);
info('A proteção global contra erros da API está ativada, proteção total' + Object.keys(rawAPI).length + 'função');
...
ServerEvents.loaded(function(event) {
    ...
    if (!autoRegisterEnabled) {
        debug('O registro automático de receita foi desativado (switch local:' + AUTO_REGISTER_RECIPES + ', mudança global:' + 
    ...
    if (configKeys.length === 0) {
        debug('A configuração da receita está vazia, realize o cadastro automático da receita...');
        autoRegisterRecipesFromArrays();
    } else {
        debug('A configuração da receita já existe (' + configKeys.length + 'entradas), pular o registro automático');
        debug('Para registrar novamente todas as receitas, use o comando "/recipe scan Registration"');
    }
});
...
info('§a✨ Shanhai Private Goods · API de controle de receita carregada!');
info('§6═══════════════════════════════════════════════════════════');
info('§b📋 API de controle de receita está pronta');
info('§7Nova função: controle de carregamento de receita §e§7- permite configuração independente se a receita está carregada');
info('§7Nova função: exportação de matriz de receitas §e§7- Exportar todas as matrizes de receitas para objetos globais');
info('§7Novos recursos: §eRegistro automático de receitas §7- Descubra e registre automaticamente todas as receitas disponíveis');
info('Comando de bate-papo §7: §e/modificação de receita §7, §e/informações de receita §7, §e/lista de receitas §7, §e/interruptor de receita §7, §e/status da receita §7, §e/Lista de receitas habilitada §7, §e/Lista de receitas desabilitada §7, §e/Diagnóstico de receita §7, §e/Registro de verificação de receita §7, §e/Ajuda de receita (também suporta prefixo!)');
info('API global §7: §eglobal.shanhaiRecipeControlAPI');
info('Requisitos de permissão §7: §cestá disponível apenas para jogadores OP');
info('§6═══════════════════════════════════════════════════════════');
