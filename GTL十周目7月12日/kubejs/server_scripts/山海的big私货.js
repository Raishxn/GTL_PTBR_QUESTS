...
(function() {
    ...
    // 环境检测
    console.log('§6[Camada de proteção de montanha e mar] §ateste ambiental aprovado §r');
    ...
    function sealAPI(apiObj, apiName) {
        ...
        try {
            ...
            console.log('§6[Camada protetora de montanha e mar] §eAviso:' + apiName + 'Use o esquema de proteção de backup §r');
        }
    ...
    console.log('§6[Camada de proteção de montanha e mar] §aA camada de proteção básica foi carregada §r');
})();
// ==================== 基础保护层结束 ====================

var Version = '2.7.4fix (sistema de log versão 2.7.3)'//主版本与日志系统版本
...
// ---------------- 日志模块 ----------------
var LOG_PREFIX = '§b[bens privados de Shanhai] §r';
...
Timer.prototype.end=function(){
    ...
    info('⏱️ ' + this.name + 'demorado:' + ms + 'ms');
    return ms;
};
    ...
    return function protectedFunction() {
        try {
            ...
            if (callCount > options.maxCallPerSecond) {
                error(`API调用频率过高: ${apiFunction.name || 'função anônima'}，当前 ${callCount}/秒，限制 ${options.maxCallPerSecond}/秒`);
                throw new Error('A frequência de chamada da API é muito alta. Tente novamente mais tarde.');
            }
            ...
            if (paramValidators) {
                for (var i = 0; i < paramValidators.length; i++) {
                    ...
                    if (validator) {
                        args[i] = validator(args[i], 'parâmetro' + (i + 1));
                    }
                }
            }
            ...
            if (options.requireOp && typeof Server !== 'undefined') {
                ...
                if (!hasOp) {
                    throw new Error('Esta API requer permissão do OP para acessar');
                }
            }
            ...
            if (options.logPerformance) {
                ...
                debug(`API ${apiFunction.name || 'função anônima'} 执行时间: ${endTime - startTime}ms`);
            }
            ...
            // 错误处理
            error(`API调用失败: ${apiFunction.name || 'função anônima'} - ${err.message}`);
            
            // 如果是验证错误或权限错误，直接抛出
            if (err.message.includes('parâmetro') || err.message.includes('Permissões') || err.message.includes('freqüência')) {
                throw err;
            }
            
            // 其他错误返回安全值
            if (options.defaultValue !== undefined) {
                warn(`API ${apiFunction.name || 'função anônima'} 出错，返回默认值: ${options.defaultValue}`);
...
function initializeProtection() {
    info('Inicializar sistema de proteção de API...');
    ...
    info('Inicialização do sistema de proteção API concluída');
}
...
function getStaticRandomText(text, seed) {
    if (typeof text !== 'string') text = 'Texto inválido';
...
function getFixedColorText(text, colorCode) {
    ...
    if (typeof text !== 'string') {
        console.error('[Bens privados de Shanhai] getFixedColorText: O texto deve ser uma string, use o texto padrão');
        text = 'Texto inválido';
    }
...
function getAlternatingColorText(text, color1, color2) {
    // 防御性编程：确保输入有效
    if (typeof text !== 'string') {
        console.error('[Bens privados de Shanhai] getAlternatingColorText: O texto deve ser uma string, use o texto padrão');
        text = 'Texto inválido';
    }
...
var newShanhaiAPI = {
        ...
        [
            ...
            function(p) { 
                if (p !== undefined && typeof p !== 'string') {
                    throw new Error('O parâmetro msg deve ser string ou indefinido, tipo real:' + typeof p);
                }
                return p;
            }
        ],
    ...
    infinityCell: function(cellString, type) {
        ...
        if (!cellString || typeof cellString !== 'string') {
            throw new Error('O parâmetro cellString deve ser uma string');
        }
        
        // 检查是否为无限单元格格式
        if (!cellString.includes('@')) {
            throw new Error('Formatos de células infinitas devem conter o símbolo @, como " expatternprovider:infinity_cell @ gtceu:hydrogen "');
        }
        
        // 解析物品字符串
        var parsed = parseItemStringCellAPI(cellString);
        if (!parsed) {
            throw new Error('Não é possível analisar o formato de célula infinita:' + cellString);
        }
        
        // 验证是否为无限单元格
        if (!parsed.id.includes('infinity_cell')) {
            warn('[shanhaiAPI.infinityCell] Aviso: o ID do item não contém "infinity_cell", mas o formato contém o símbolo @:' + cellString);
        }
...
if (global.shanhaiAPI && typeof global.shanhaiAPI === 'object') {
    ...
    info('A API Shanhai existente foi mesclada: mesclar' + mergedCount + 'atributos, substituir' + overriddenCount + 'atributos');
} else {
    info('Inicialize a nova API Shanhai');
}
...
global.shanhaiRecipeAPI = {
    ...
    reset: function() {
        ...
        typeFailed = 0;
        info('As estatísticas da receita foram redefinidas');
    },
     ...
     *     console.log('Shanhai Private Goods foi carregado');
     * }
     */
    isLoaded: function() {
    ...
    findRecipeById: function(id) {
        ...
        console.log('§e[shanhaireicipeAPI] shanhaiRecipeControlAPI.findRecipeById não está disponível');
        return null;
    },
    ...
    getRecipeDetails: function(recipeOrId) {
        if (typeof recipeOrId === 'string') {
            ...
            if (!result) return 'Sem informações de receita';
     ...
     * console.log('Defina os resultados do status ativado:', success);
     */
     ...
     * console.log('Defina os resultados do valor padrão:', success);
     */
    setRecipeDefault: function(recipeId, defaultValue) {
        console.log('A função setRecipeDefault de §e[Shanhai Recipe API] foi removida (limpeza de código duplicado)');
        return false;
    },
     ...
     * console.log('Valor padrão da receita:', defaultValue);
     */
    getRecipeDefault: function(recipeId) {
        console.log('A função getRecipeDefault de §e[Shanhai Recipe API] foi removida (limpeza de código duplicado)');
        return null;
    },
     ...
     *     'recipe_c': false
     * });
     * console.log('Resultados da configuração em lote:', result);
     */
    batchSetRecipeDefaults: function(defaults) {
        console.log('A função §e[Shanhai Recipe API] batchSetRecipeDefaults foi removida (limpeza de código duplicado)');
     ...
     * console.log('Número total de valores padrão:', Object.keys(allDefaults).length);
     */
    getAllRecipeDefaults: function() {
        console.log('A função getAllRecipeDefaults de §e[Shanhai Recipe API] foi removida (limpeza de código duplicado)');
        return {};
    },
    ...
    initializeMissingDefaults: function(defaultValue) {
        console.log('A função §e[Shanhai Formula API] initializeMissingDefaults foi removida (limpeza de código duplicado)');
     ...
     * console.log('Resultado da redefinição:', success);
     */
    resetRecipeLoadConfigToDefaults: function() {
        if (typeof global.shanhaiRecipeLoadConfig !== 'undefined') {
            global.shanhaiRecipeLoadConfig = {};
            console.log('§a[API de receitas Shanhai] A configuração de carregamento de receitas foi redefinida para o valor padrão');
            return true;
        }
        console.log('§e[API de receita Shanhai] A configuração de carregamento de receita é indefinida, não há necessidade de redefinir');
        return false;
    },
    ...
    findRecipeInAllSources: function(recipeId) {
        ...
        if (global.shanhaiRecipeInfoCollector) {
            ...
            if (collected) {
                return { recipe: collected, source: 'Colecionador de Receitas' };
            }
        }
        ...
        for (var i = 0; i < recipeArrays.length; i++) {
            ...
            if (arr.data && Array.isArray(arr.data)) {
                ...
                if (found) {
                    return { recipe: found, source: 'Matriz de receitas:' + arr.name };
                }
            }
        }
    ...
    formatRecipeInfo: function(sender, result, recipeId) {
        ...
        sender.tell('§6═══════ Informações da receita ═══════');
        ...
        if (recipe.defaultEnabled !== undefined) {
            sender.tell(`§7默认启用: ${recipe.defaultEnabled ? '§aé' : '§cNão'}`);
        }
        sender.tell('§6═══════════════════════');
    },
...
var DShanhaiSDA = {
    create: function(name) {
        var api = Java.loadClass('com.dishanhai.gt_shanhai.api.DShanhaiNBTAPI');
        return {
            _items: [], _vc: [], _name: name || 'matriz de super disco', _lore: [],
...
ServerEvents.recipes(function(e) {
    ...
    function setLocalRecipeDefault(recipeId, defaultValue) {
        if (typeof recipeId !== 'string' || !recipeId.trim()) {
            warn('setLocalRecipeDefault: o ID da receita deve ser uma string válida');
            return false;
        }
        if (typeof defaultValue !== 'boolean') {
            warn('setLocalRecipeDefault: O valor padrão deve ser booleano (verdadeiro/falso)');
            return false;
        }
        localRecipeDefaults[recipeId] = defaultValue;
        debug('✅ Defina o valor padrão local da receita:' + recipeId + ' = ' + defaultValue);
        return true;
    }
    ...
    function removeLocalRecipeDefault(recipeId) {
        if (localRecipeDefaults.hasOwnProperty(recipeId)) {
            ...
            debug('🗑️ Excluir o valor padrão local da receita:' + recipeId);
            return true;
        }
        return false;
    }
    ...
    global.shanhaiAPI.clearLocalDefault = function(recipeId) {
        ...
        try {
            ...
            if (totalRemoved > 0) {
                debug('Padrões locais apagados para receitas:' + recipeId + '(comum' + totalRemoved + 'variações)');
            }
            return totalRemoved > 0;
        } catch (e) {
            error('Erro ao limpar os padrões locais:' + e);
            return false;
        }
    };
    ...
    function generateRecipeStatistics() {
        ...
        if (totalRecipes === 0) {
            info('📊 Estatísticas de receita: nenhuma informação de receita coletada');
            return;
        }
    ...
    info('Matriz de receitas exportada para objeto global');
...
ServerEvents.recipes(e => {
    var timer_mek_remove = new Timer('Módulo de remoção de receita do Mekanism');
    info('📝 Comece a processar a remoção da receita Mekanism/Allthemodium...');
    ...
    if (Platform.isLoaded('mekanism')){
        ...
        removeList.forEach(item => {
            try {
                e.remove(item);
                debug(`删除配方: input=${item.input}, mod=${item.mod || 'nenhum'}`);
            } catch(err) {
        ...
        outputRemoveList.forEach(item => {
            try {
                e.remove(item);
                debug(`删除输出配方: output=${item.output}, mod=${item.mod || 'nenhum'}`);
            } catch(err) {
...
ServerEvents.tags('item', e => {
    var timer_item_tags = new Timer('Modificação do rótulo do item');
    info('🏷️ Modifique a inicialização do rótulo do item...');
    ...
    try {
        ...
        debug('Modificação da tag concluída');
    } catch(err) {
...
ServerEvents.tags('fluid', e => {
    var timer_fluid_tags = new Timer('Modificação do rótulo do fluido');
    info('💧 Comece a modificar rótulos de fluidos...');
...
ServerEvents.tags('item', event => {
    var timer_batch_item_tags = new Timer('Exclusão de rótulo de item em massa');
    info('🗑️ Comece a excluir tags de itens em lotes...');
...
function parseItemStringCellAPI(str) {
    if (!str || typeof str !== 'string') {
        throw new Error('String de item inválida:' + str);
    }
...
function parseFluidStringCellAPI(str) {
    if (!str || typeof str !== 'string') {
        throw new Error('String de fluido inválida:' + str);
    }
...
function validateCellRecipe(itemList, inputItems) {
    ...
    if (!itemList || !Array.isArray(itemList) || itemList.length === 0) {
        errors.push('A lista de itens não pode ficar vazia');
    }
...
function checkCompatibility(expectedVersion) {
    if (!expectedVersion) {
        throw new Error('O parâmetro expectVersion não pode estar vazio');
    }
...
function measurePerformance(fn, context) {
    return function() {
        ...
        try {
            ...
            if (duration > 1000) {
                warn('[API de célula de 256k] Aviso de desempenho:' + (fn.name || 'função anônima') + 'Tempo de execução' + duration + 'ms');
            }
            return result;
        } catch (err) {
            var duration = Date.now() - start;
            error('[API de célula de 256k] Erro de desempenho:' + (fn.name || 'função anônima') + 'implementar' + duration + 'Falha após ms:' + err.message);
            throw err;
        }
    };
}
...
function buildCellNBTCellAPI(items, cellName, lore) {
    try {
        ...
        error('[CellAPI] Falha na compilação do Java NBT:' + e.message);
        return '';
    }
}
...
function addCellAssemblerRecipeCellAPI(recipeId, cellName, itemList, lore, inputItems, inputFluids, circuit, duration, eut, defaultEnabled) {
    ...
    try { 
        ...
        if (_registeredCellRecipes.has(recipeId)) {
            warn('Receita [API de célula 256k]' + recipeId + 'Já existe, pule o registro');
            return false;
        }
        ...
        if (!recipeId || !recipeId.includes(':')) { 
            throw new Error('O formato do ID da receita está incorreto, deve usar o formato namespace:path'); 
        } 
        ...
        return function(gtceu) { 
            ...
            info('[API de célula 256k] Receita gerada:' + recipeId); 
            info('Pacote de itens:' + cellName + ' (' + itemList.length + 'Unid)'); 
            info('Tensão:' + eut + 'EU/t, demorado:' + duration + ' ticks'); 
            info('Circuito:' + circuit + ', insira o item:' + inputItems.length + 'tipo');
            
            return true; 
        }; 
        
    } catch (err) { 
        error('[256k Cell API] Falha na geração da receita (' + recipeId + '): ' + err.message); 
        return null; 
    } 
}
...
function getCellContentCellAPI(cellItem) {
    try {
        ...
        error('[256k Cell API] Falha ao analisar o conteúdo do pacote de itens:' + err.message);
        return [];
    }
}
...
var CellAPI = {
    ...
    addBatchRecipes: function(recipes, gtceu) {
        if (!recipes || !Array.isArray(recipes)) {
            throw new Error('O parâmetro receitas deve ser um array');
        }
        if (!gtceu) {
            throw new Error('O parâmetro gtceu não pode ficar vazio');
        }
        ...
        recipes.forEach(function(recipe) {
            try {
                ...
                error('[256k Cell API] Falha no registro em lote de receitas (' + (recipe.id || 'desconhecido') + '): ' + err.message);
                failed++;
            }
        });
        info('[CellAPI] Registro em lote concluído: sucesso' + success + ', falhar' + failed);
    ...
    infinityCell: function(itemString, cellName, lore) {
        ...
        if (!itemString || typeof itemString !== 'string') {
            throw new Error('O parâmetro itemString deve ser uma string');
        }
        ...
        if (!parsed) {
            throw new Error('Não é possível analisar o formato de célula infinita:' + itemString);
        }
        
        // 验证是否为无限单元格
        if (!parsed.id.includes('infinity_cell')) {
            warn('[CellAPI.infinityCell] Aviso: o ID do item não contém "infinity_cell", mas o formato contém o símbolo @:' + itemString);
        }
        
        // 构建NBT标签
        var itemList = [itemString];
        var nbt = buildCellNBTCellAPI(itemList, cellName || 'células infinitas', lore || ['Células infinitas §6', '§7Itens internos:' + parsed.innerId]);
    ...
    addInfinityCellRecipe: function(recipeId, infinityCellString, inputItems, inputFluids, circuit, duration, eut, gtceu) {
        ...
        if (!infinityCellString || typeof infinityCellString !== 'string') {
            throw new Error('O parâmetro infinitoCellString deve ser uma string');
        }
        ...
        if (!parsed) {
            throw new Error('Não é possível analisar o formato de célula infinita:' + infinityCellString);
        }
        
        // 验证是否为无限单元格
        if (!parsed.id.includes('infinity_cell')) {
            warn('[CellAPI.addInfinityCellRecipe] Aviso: o ID do item não contém "infinity_cell", mas o formato contém o símbolo @:' + infinityCellString);
        }
        
        // 生成单元格名称和描述
        var cellName = 'Células infinitas:' + (parsed.innerId || 'desconhecido');
        var lore = [
            'Células infinitas §6',
            '§7Itens internos:' + (parsed.innerId || 'desconhecido'),
            'Quantidade §7: Ilimitada'
        ];
        ...
        if (result) {
            info('[CellAPI.addInfinityCellRecipe] Receita de célula infinita registrada:' + recipeId + ' (' + infinityCellString + ')');
        }
    ...
    // 版本信息
    version: '1.0.0',
    author: 'Montanhas e mares são eternos/dishanhai'
};

// 导出到全局
if (typeof global !== 'undefined') {
    global.CellAPI = CellAPI;
    info('[256k Cell API] Carregado, versão' + CellAPI.version);
    
    // 检查依赖
    let deps = checkDependenciesCellAPI();
    if (!deps.allLoaded) {
        info('[API de célula 256k] Dependências ausentes:');
        if (!deps.ae2) info('- AE2 não carrega');
        if (!deps.gtceu) info('- GTCEu não carrega');
    } else {
        info('[256k Cell API] Todas as dependências foram atendidas');
    }
}
...
info('[API de 256 mil células] Número de recargas a quente:' + global.__kubejs_cell_api_reload_count);
...
function getRecipeDetails(recipe) {
    if (!recipe) return 'Sem informações de receita';
    let details = 'ID: ' + recipe.id + 'tipo:' + recipe.type + '\n';
    if (recipe.itemInputs) details += 'Entrada de item:' + JSON.stringify(recipe.itemInputs) + '\n';
    if (recipe.inputFluids) details += 'Entrada de fluido:' + JSON.stringify(recipe.inputFluids) + '\n';
    if (recipe.itemOutputs) details += 'Saída do item:' + JSON.stringify(recipe.itemOutputs) + '\n';
    if (recipe.outputFluids) details += 'Saída de fluido:' + JSON.stringify(recipe.outputFluids) + '\n';
    if (recipe.EUt !== undefined) details += 'EU/t: ' + recipe.EUt + '\n';
    if (recipe.duration !== undefined) details += 'demorado:' + recipe.duration + '\n';
    if (recipe.circuit !== undefined) details += 'Circuito:' + recipe.circuit + '\n';
    if (recipe.notConsumable !== undefined) details += 'Não consumíveis:' + recipe.notConsumable + '\n';
    return details;
}
...
function saveConfigToFile(config) {
    try {
        if (typeof JsonIO !== 'undefined' && typeof JsonIO.write === 'function') {
            ...
            console.log('§a[Reparo de configuração] Configuração salva:' + Object.keys(config).length + 'Unid');
            return true;
        }
    } catch (err) {
        console.log('§c[Reparo de configuração] Falha ao salvar a configuração:' + err.message);
    }
    return false;
}
...
ServerEvents.loaded(event => {
    ...
    (function() {
        ...
        function collectRecipeDefaultsFromCollector() {
            ...
            if (!collector || typeof collector !== 'object') {
                console.log('§e[Correção de configuração] O coletor de receitas não existe');
                return null;
            }
            
            var totalKeys = Object.keys(collector).length;
            console.log('§7[Correção de configuração] Total de chaves do coletor:' + totalKeys);
            ...
            console.log('§a[Correção de configuração] Obtido do coletor' + count + 'valor padrão da receita');
        ...
        function syncAllRecipesToConfig(forceOverwrite) {
            ...
            console.log('§6[Correção de configuração] Comece a sincronizar todas as receitas com arquivos de configuração...');
            
            if (typeof global !== 'undefined' && global.shanhaiRecipeConfigJustReset === true) {
                console.log('§e[Correção de configuração] Sinalizador de redefinição detectado, sincronização ignorada');
                return false;
            }
            
            var allDefaults = collectRecipeDefaultsFromCollector();
            if (!allDefaults || Object.keys(allDefaults).length === 0) {
                console.log('§e[Correção de configuração] O coletor está vazio e não pode ser sincronizado');
                return false;
            }
            ...
            for (var key in allDefaults) {
                if (allDefaults.hasOwnProperty(key)) {
                    if (finalConfig[key] === undefined) {
                        ...
                        addedCount++;
                        console.log('§7[Correção de configuração] Adicionar nova receita:' + key + ' = ' + (allDefaults[key] ? 'habilitar' : 'Desativar'));
                    } else {
                        skippedCount++;
                        // 已存在配置，保留用户设置，不覆盖
                    }
            ...
            console.log('§a[Correção de configuração] Novo' + addedCount + 'receita, reservada' + skippedCount + 'configuração de usuário individual');
            
            if (addedCount > 0) {
                saveConfigToFile(finalConfig);
            } else {
                console.log('§a[Reparo de configuração] A configuração está atualizada, total' + Object.keys(finalConfig).length + 'receitas');
            }
        ...
        function trySync(e) {
            attempts++;
            console.log('§7[CORRECÇÃO DE CONFIGURAÇÃO] Tente sincronizar receitas (seção' + attempts + '/' + maxAttempts + 'Segunda categoria)');
            
            var collector = global.shanhaiRecipeCollector || global.shanhaiRecipeInfoCollector;
            var collectorSize = collector ? Object.keys(collector).filter(function(k) { return k !== '_statistics'; }).length : 0;
            
            if (collectorSize > 0) {
                console.log('O coletor §a[correção de configuração] já existe' + collectorSize + 'receitas');
                ...
                console.log('§e[Correção de configuração] Máximo de tentativas atingido, coletor ainda vazio');
            }
        }
        
        console.log('§6[Correção de configuração] Correção de persistência de configuração carregada');
    ...
    if (typeof recipes !== 'undefined') global.recipes = recipes;
    if (typeof recipes_electrolyzers !== 'undefined') global.recipes_electrolyzers = recipes_electrolyzers;
    info('Matriz de receitas exportada para objeto global (ServerEvents.loaded)');
    ...
    if (global.__shanhai_guard__) {
        ...
        if (global.shanhaiAPI) {
            ...
            info('§6[Camada de proteção de montanha e mar] §aA API principal foi selada e protegida §r');
        }
        if (global.shanhaiRecipeAPI) {
            guard.sealAPI(global.shanhaiRecipeAPI, 'shanhaiRecipeAPI');
            info('§6[Camada protetora de montanha e mar] §aFórmula API foi selada e protegida §r');
        }
        if (global.shanhaiRecipeControlAPI) {
            guard.sealAPI(global.shanhaiRecipeControlAPI, 'shanhaiRecipeControlAPI');
            info('§6[Camada de proteção de montanha e mar] §aAPI de controle de fórmula foi selada e protegida §r');
        }
    }
