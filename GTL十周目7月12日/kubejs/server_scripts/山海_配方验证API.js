...
var LOG_PREFIX = '§d[API de verificação de receita] §r';
...
function getGtrObject() {
    ...
    for (var i = 0; i < possibleGtrSources.length; i++) {
        if (possibleGtrSources[i]) {
            debug('da fonte' + i + 'Obtenha o objeto gtr');
            return possibleGtrSources[i];
        }
    }
    
    warn('Não foi possível obter o objeto gtr, algumas funções de verificação podem ser limitadas');
    return null;
}
...
function getAllRecipeArrays() {
    ...
    // （以'recipe'、'recipes'、'fórmula'等关键词结尾的变量）
    var recipeKeywords = ['recipe', 'recipes', 'fórmula', 'ass', 'universal', 'sup'];
