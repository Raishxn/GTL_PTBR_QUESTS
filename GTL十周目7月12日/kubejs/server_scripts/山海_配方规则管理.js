...
ServerEvents.recipes(function(event) {
  // 诊断: 打印可用的山海全局绑定
  console.log('[Regras da Receita Shan Hai] chaves globais:', Object.keys(global).filter(k => k.toLowerCase().includes('shanhai') || k.toLowerCase().includes('recipe')))
  console.log('[Regras de receitas Shanhai] tipo de receitas Shanhai:', typeof ShanhaiRecipes, 'typeof global.ShanhaiRecipes:', typeof global.ShanhaiRecipes)

  var R = global.ShanhaiRecipes || ShanhaiRecipes
  if (!R) {
    console.error('[Regras de receitas de Shanhai] ShanhaiRecipes não está definido, pule todas as regras')
    return
  }
  ...
  // console.info('[山海] encontrado' + ids.length + 'Fórmula Nano Enxame')

  console.info('[Shan Hai·Rule Management] As regras de receita foram carregadas')
})
