...
function consume(player, hand) {
    ...
    if (tt > 0) {
        player.setStatusMessage("Horários disponíveis:" + (tt - 1))
    } else {
...
PlayerEvents.tick(event => {
    if (event.player.age % 20 == 0) {
        ...
        if (tt > 20) {
            if (tti) {
                event.player.setStatusMessage("Horários disponíveis:" + 20)
            }
        } else {
            event.player.persistentData.putInt("tt", tt + 1)
            if (tti) {
                event.player.setStatusMessage("Horários disponíveis:" + (tt + 1))
            }
        }
    }
})
