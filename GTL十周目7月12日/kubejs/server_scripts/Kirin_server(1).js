...
;(function() {
	...
	console.log('Status de carregamento de mercadorias privadas:')
	...
	if (loadedAddons.gtladd3) {
		console.warn('GtlAdditions3.0+ detectado, este produto privado não foi testado quanto à compatibilidade, continuar carregando pode causar problemas!')
	}
	//所有流体
	...
	if (global.kirin.enableSafeFixes) {
		...
		BlockEvents.rightClicked(Ingredient.of(/gtceu:*_parallel_hatch/), (event) => {
			...
			if (currentParallel > maxAllowed) {
				player.tell(Text.aqua('Você está tentando abrir um compartimento de controle paralelo com um número paralelo que excede o número máximo de controles paralelos para esse nível de compartimento de controle paralelo. Tem certeza de que deseja fazer isso?'))
				player.tell(Text.red('Isto fará com que o número paralelo do compartimento de controle paralelo seja redefinido para o número paralelo máximo do compartimento de controle paralelo naquele nível!'))
				player.tell(Text.darkGray('Segure o stick com a mão principal para ignorar este aviso e abra a câmara de controle paralela.'))
				event.cancel()
			}
		})
		//大并行控制仓原样掉落
			...
			if (currentParallel > maxAllowed) {
				...
				if (loadedAddons.thetornproductionline) {
					...
					if (currentParallel === maxAllowed * 4) {
						compressionText = 'Uma camada de compressão'
					} else if (currentParallel === maxAllowed * 16) {
						compressionText = 'Compressão dupla'
					} else if (currentParallel === maxAllowed * 64) {
						compressionText = 'compressão tripla'
					}
					let loreText = `"$ {compressionText} Número real de paralelos: $ {currentParallel} Coloque com cuidado O paralelismo será redefinido após minerar/clicar"`
	...
	ServerEvents.recipes((event) => {
		...
		if (global.kirin.enableSafeFixes) {
			...
			// 无限编程电路元件包
			{
				...
				let nbtString = `{RepairCost:0,amts:[L;${amtsArray}],display:{Name:'{"text":"Pacote de componentes de circuito de programação infinita §r"}'},ic:33L,internalCurrentPower:20000.0d,keys:[${keysString}]}`
